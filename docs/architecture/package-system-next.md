...
6. **Compiled-report aggregate closure** (Stage C, complete — see [post-c203e7a7-aggregate-closure.md](../reviews/post-c203e7a7-aggregate-closure.md)) — persists `AdaptedReportDTO` as a first-class artifact, binds it to `CompiledReportArtifactDTO` by a real `ArtifactRef`, and adds `zeromodel.artifacts.aggregate` to resolve and cross-validate the complete four-object aggregate (adapted report, `ScoreTable`, `LayoutRecipe`, `VPMArtifact`) — proving the collection describes one coherent report, not merely that each object's own digest is valid. Also fixes `zeromodel.trust`'s `signature_envelope_id` to be a content-derived identity rather than the raw signature hex.
