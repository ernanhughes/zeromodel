...
- **`zeromodel.search` does not exist.** `TraversalRule` in Navigation is a deliberate seam a future Search package can implement with similarity-driven rules against the same protocol; Navigation itself defines no similarity or relevance concept. Search is planned, not started, and should not begin before the Stage 1A hardening and Stage B adapter work above are complete — those items establish the identity/closure guarantees Search would otherwise have to multiply across an entire corpus.
