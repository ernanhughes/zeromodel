...
> ZeroModel turns scored data into deterministic, inspectable Visual Policy Map artifacts. Those artifacts preserve source mapping, deterministic identity, declared parent lineage, renderable fields, bundle round-trips, state-addressed policy lookup over closed enumerable policies, non-action criticality and decision-margin evidence for Q-bearing policies, exhaustive named row-level property checking, linked verification artifacts with exact counterexamples, named policy views over the same dense source table, deterministic spatially optimized view profiles for an explicit top-left mass objective, temporal decision manifolds over consistent scored panel sequences, small consumer decisions without model invocation, scored learning traces, checkpoint-level training progress artifacts, dependency-light tracker adapters, and critic/evidence/policy risk artifacts for inspection.
...
1. **Dynamic policy verification** — add transition-system properties and compare finite scans with an SMT-backed checker on the same fixture.
2. **Criticality-weighted representation benchmark** — compare uniform and criticality-weighted allocation under a fixed artifact budget.
3. **Human inspection study** — compare source-order, criticality-first, table, and log views for counterexample detection time and accuracy.
4. **PNG metadata round-trip** — embed manifest/provenance in PNG text chunks and prove decode returns the same artifact ID.
5. **Hierarchy traversal benchmark** — generated corpus, child pointers, resolver, traversal harness, raw timing report.
6. **Edge benchmark** — minimal no-model gate/policy lookup in a tiny runtime, with memory and timing measurements.
7. **Decision manifold benchmark** — compare manifold inflection frames against known synthetic change points and random/scalar baselines.
8. **Signed verification lineage** — combine artifact/verification identities with Sigstore or in-toto attestations.
9. **Sanitized critic exports** — real critic or RAG verifier outputs checked into tests or docs.
10. **Sanitized real tracker exports** — real TensorBoard, W&B, and Trackio exports checked into tests or docs.
11. **Examples package** — search triage, safety gate, anomaly toy dataset, code review trace.
