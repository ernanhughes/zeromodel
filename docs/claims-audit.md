...
| Deterministic reproducible provenance. | Artifacts include source and recipe digests, parents, provenance payload, cached identity bytes, and identity mismatch validation. Tests cover golden artifact ID, artifact ID determinism, and tamper rejection. | **Validated for artifact identity** | Add tests for parent lineage, multi-artifact provenance graphs, and replay from bundles. |
...
| Multi-metric, multi-view by design. | `ScoreTable` supports multiple metrics; `LayoutRecipe` supports different ordering/column recipes; `ViewProfile` makes named policy views explicit; `SpatialOptimizer` can derive a metric-weight profile for a geometric objective; `DecisionManifold` tracks optimized views over time. | **Validated core mechanism** | Next proof: benchmark learned views and manifold inflections against manual/source-order views and random frame sampling on task outcomes. |
...
| Traceable “thought” / 40+ levels. | Current code has provenance, controller signals, learning traces, training progress artifacts, tracker export adapters, end-to-end fixtures, critic evidence artifacts, view profiles, spatial optimization, policy lookup, and decision manifolds, but no router frame chain or step graph implementation. | **Reframe / not validated** | Use “traceable artifact lineage,” “multi-view dense artifact,” “state-addressed policy lookup,” “spatially optimized view,” “temporal decision manifold,” “learning trace,” “training progress artifact,” “tracker export adapter,” “critic evidence artifact,” or “research fixture” when those are the actual artifacts. Avoid “thought” unless strictly metaphorical. |
| Human-compatible explanations. | Cell/source mapping, policy-lookup decisions, SVG/PNG rendering, view profiles, spatial optimization summaries, temporal manifold summaries, and critic explanation metadata allow inspection. | **Implemented / thin evidence** | Reframe as “inspectable evidence mapping.” Explanation quality requires user-facing viewer tests and examples. |
...
| Works with your stack. | `metrics.pack_metrics()` accepts common aliases and builds score tables from metric rows; `zeromodel.adapters` ingests exported tracker logs; `zeromodel.critic` ingests Writer-style critic outputs; `zeromodel.views` creates policy views over those tables; `zeromodel.spatial` derives view profiles for top-left objectives; `zeromodel.manifold` summarizes consistent panel sequences. | **Implemented for exported telemetry, critic traces, dense views, spatial objectives, and temporal panels** | Add pandas adapters, real fixture exports, sanitized Writer critic outputs, and optional live SDK integrations if broader wording is needed. |
| Great fits: anomaly detection, safety gates, code review traces, search triage, closed-world policy lookup. | Current package has primitives and early domain examples for training, learning, critic traces, multi-view artifacts, spatial optimization, temporal manifolds, and a tiny arcade-shooter policy lookup demo. | **Implemented / thin evidence** | Create stronger examples for anomaly detection, safety gates, code review traces, and search triage before promoting them as out-of-the-box fits. |
...
- “improves decision accuracy” except for narrow tested fixtures such as the closed-world arcade-shooter policy lookup example
...
3. **Edge benchmark** — minimal no-model gate/policy lookup in a tiny runtime, with memory and timing measurements.
4. **Optimized view-profile benchmark** — learn metric weights for a task and compare against manual/source-order views.
5. **Decision manifold benchmark** — compare manifold inflection frames against known synthetic change points and random/scalar baselines.
6. **Lineage/provenance replay** — parent artifact chain and deterministic replay from bundles.
7. **Sanitized critic exports** — real Writer critic or RAG verifier outputs checked into tests or docs.
8. **Learning trace domain examples** — agent trace learning, RAG correction learning, Writer evaluator learning.
9. **Sanitized real tracker exports** — real TensorBoard, W&B, and Trackio exports checked into tests or docs.
10. **Examples package** — search triage, safety gate, anomaly toy dataset, code review trace.
11. **Website claim labels** — mark claims as implemented, benchmarked, or roadmap in the static site.
12. **Policy-lookup portability** — C/Rust/MicroPython consumer over a committed policy artifact, with timing and memory report.

## Bottom line

The cleaned repo now validates the core abstraction, adds named policy views over dense source tables, derives spatially optimized view profiles for an explicit top-left mass objective, summarizes temporal decision manifolds for consistent scored panel sequences, adds a concrete scored-trace definition of visible learning, adds checkpoint-level model-training progress artifacts, can ingest common tracker export files, includes deterministic end-to-end fixtures, can artifactize critic/evidence/policy scores, and includes a tested closed-world policy-lookup demo. It does **not** yet validate the strongest blog-scale performance claims, automatic semantic view learning, open-world decision accuracy improvement, universal forty-step discovery, or real-world hallucination detection. The next work should benchmark optimized views/manifolds and connect spatial calculus to real-world evidence without overclaiming.
