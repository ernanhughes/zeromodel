...
## Remediation status (Stage A2)

This is an append-only status update; the findings above are left exactly as originally written. See [post-split-stage-a2-validation.md](post-split-stage-a2-validation.md) for the full account, [post-split-stage-a2-test-counts.csv](post-split-stage-a2-test-counts.csv) for counts, [post-split-stage-a2-api-manifest-validation.md](post-split-stage-a2-api-manifest-validation.md) for the manifest rewrite, and [post-split-stage-a2-test-ownership-changes.csv](post-split-stage-a2-test-ownership-changes.csv) for every test that was split, moved, or reclassified.

Stage A2 ("verification completeness and contract restoration") addressed **F4, F5, F6, F7, H2, H3 (partially), H4 (partially), M3, M6, M7 (partially), M9, L4**:

| finding | status | evidence |
|---|---|---|
| F4 (SQLAlchemy missing from repository-wide quality enforcement) | **resolved** | `packages/sqlalchemy/src`/`tests` added to `check_quality.py`'s Ruff/mypy/quality-limit paths and root `mypy_path`; the real mypy defect this gap was masking (`Result[Any]` has no `rowcount`) is fixed with an `isinstance(result, CursorResult)` runtime-validated narrowing; `python -m mypy packages/sqlalchemy/src packages/sqlalchemy/tests` passes with 0 errors and no package-wide suppression |
| F5 (canonical fast suite excludes package-local tests) | **resolved** | `scripts/run_fast_tests.py` rewritten to explicitly pass all six `packages/*/tests` roots plus `tests`/`integration_tests`, using a `not slow and not external and not research` marker expression instead of directory omission; verified 794 passed / 0 failed across 874 collected in 100s (120s budget) |
| F6 (public API manifest is a 6-row placeholder) | **resolved** | `write_public_exports()` rewritten to introspect real `__all__` per package in an isolated clean-venv environment; manifest now has 211 real per-symbol rows, verified deterministic and complete against every package's actual `__all__` |
| F7 (documented core golden identity conflicts with enforced evidence) | **resolved** | `package-core-validation-1.0.13.md` corrected to the enforced test's value (`32f801671139b73e349c756570c27c06d39c422a4d9a277782e1c997a473083b`); `package-integration-validation-1.0.13.md` already agreed; a regression test now enforces both stay in sync with the test |
| H2 (package-local production suites not part of the canonical fast signal) | **resolved** | same fix as F5 - all six package-local suites are now explicitly part of `run_fast_tests.py` |
| H3 (public API contract not represented accurately) | **resolved** | same fix as F6 |
| H4 (production invariants moved into research with benchmark tests) | **partially resolved** | the most concretely-specified case (`test_video_policy.py`, 10 of 11 tests) fully split into `packages/video/tests/test_video_policy_reader_contracts.py` with a new synthetic fixture (no `examples`/`research` import); the other 3 named files were fixed (collection bugs, explicit `research` marker) and classified in detail, but 2 of them (facade/closure-kernel architecture-layering tests) were left in `research/` as a documented, not-executed relocation recommendation, since they don't affect the production gate either way |
| M3 (claims-audit workflow watches deleted source paths) | **resolved** | `claims-audit.yml` now reacts to `packages/**`, `tests/**`, `integration_tests/**`, `scripts/**`, `README.md`, `package-boundaries.toml` |
| M6 (integration/research marker handling incomplete) | **resolved** | `tests/conftest.py` now recognizes `integration_tests` (not just `integration`) by path, and auto-tags `research` by path/filename/prefix in addition to the pre-existing explicit `pytestmark` declarations |
| M7 (test ownership remains directory-driven in places) | **partially resolved** | the `test_video_action_set_*` filename prefix in `tests/` was reclassified from auto-`integration` to auto-`research` after confirming via Stage A1's own classification that all 6 matching files are genuinely research benchmarks/evidence, not production integration (this is what surfaced a real, previously-masked test - `test_video_action_set_benchmark.py` - failing with a genuine `observation record keys mismatch` schema-drift bug in `packages/video/src/.../observation_dto.py`, now correctly excluded from the production gate rather than silently blocking it); full directory-vs-behavior reconciliation beyond what blocked the production contract was not attempted |
| M9 (release validation doesn't report complete production test coverage) | **resolved** | `validate_release_candidate.py` now writes `docs/architecture/package-release-test-layers-1.0.13.json`, distinguishing source-tree fast tests, package-local source tests, cross-package integration tests, installed-wheel smoke results, and research (explicitly excluded by policy, not silently absent) |
| L4 (inconsistent test surfaces across tooling) | **resolved** | `.vscode/settings.json` already matched `run_fast_tests.py`'s six package roots after an earlier commit; added a trailing newline and a regression test (`tests/test_developer_tooling_consistency.py`) keeping the two in sync going forward |

**Correction (Stage A2.1):** the paragraph originally here claimed `OBSERVATION_RECORD_KEYS` was missing fields relative to `ObservationDTO` and characterized this as an undiscovered production defect. A full trace of `ObservationDTO.from_record()`/`to_record()` in Stage A2.1 proved that claim wrong: `OBSERVATION_RECORD_KEYS` is intentionally the top-level JSON record schema, narrower than the full dataclass by design (five fields live inside the record's `metadata` sub-dict; two more are derived from the optional `pixels` key or supplied externally) - this is correct production code, not drift. The actual defect was a stale, incomplete fixture in `tests/test_video_action_set_benchmark.py` itself (missing 8 required keys), which was corrected in Stage A2.1, along with a permanent regression test (`packages/video/tests/test_observation_record_schema.py`) documenting exactly which dataclass fields are excluded from the record schema and why. See [post-split-stage-a2-validation.md](post-split-stage-a2-validation.md)'s "Stage A2.1 addendum" section for the full evidence trail. No production code behavior was changed by this correction.

Roughly 40 new repository-tooling and package-local regression tests were added across this stage (see `docs/reviews/post-split-stage-a2-validation.md`'s file list) to guard every fix above against recurrence. No package runtime behavior changed except the two justified SQLAlchemy typing fixes (one production, `isinstance(result, CursorResult)`; six test-only). No test was deleted before its behavior was mapped; every split/moved/rewritten test is recorded in `docs/reviews/post-split-stage-a2-test-ownership-changes.csv` with its original and new location, asserted behavior, and reason. Nothing was published, tagged, or version-bumped.
