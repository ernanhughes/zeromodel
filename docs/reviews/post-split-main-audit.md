...
A newly-discovered, out-of-scope bug surfaced by this stage's own fixes: `packages/video/src/zeromodel/video/domains/video_action_set/observation_dto.py`'s `OBSERVATION_RECORD_KEYS` validation tuple is missing several fields present on the current `ObservationDTO` dataclass (`benchmark_seed_digest`, `episode_plan_digest`, `matrix_blob_id`, `provider_observation_descriptor`, `provider_observation_digest`, `operation_chain`) - a schema-validation/dataclass drift that made `tests/test_video_action_set_benchmark.py` (a genuine research file, now correctly excluded from the production gate per the M7 fix above) fail with `VPMValidationError: observation record keys mismatch`. This was not fixed in this stage - it is a production-code defect discovery, not a verification-tooling gap, and belongs to a dedicated follow-up.
