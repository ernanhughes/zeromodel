...
OUTPUT_DIR_V2 = (
    REPO_ROOT / "docs" / "results" / "video-discriminative-local-evidence-v2"
)
OUTPUT_DIR_V3 = (
    REPO_ROOT / "docs" / "results" / "video-discriminative-local-evidence-v3"
)
...
FINAL_SEED_DIGEST_V2 = (
    "sha256:d740c41178cfe660b3d10d680eafc1f6e20177ba2968ebb5057aa99abd2652cd"
)
...
STAGE2_BENCHMARK_DIGEST = (
    "sha256:589bb074e1b53b06657cfb75bf7b8d67eae43cc5f76e7237ab07f23ccca49c75"
)
STAGE2_SPLIT_DIGEST = (
    "sha256:d25b694b3cce93bf93f58239163331f3f6370d32a2b5cce53b4541902b0f8c23"
)
...
def _translate(
    frame: np.ndarray, *, dx: int = 0, dy: int = 0, fill: int = 0
) -> np.ndarray:
...
def _scale_intensity(
    frame: np.ndarray, *, numerator: int = 100, offset: int = 0
) -> np.ndarray:
    scaled = np.round(frame.astype(np.float32) * (float(numerator) / 100.0)) + float(
        offset
    )
...
def _mask_box(
    frame: np.ndarray, *, top: int, left: int, height: int, width: int, value: int
) -> np.ndarray:
...
def _blend_region(
    ...
    *,
    ...
    result = np.array(frame, copy=True)
    result[top : top + height, left : left + width] = other[
...
class Stage3Benchmark:
    ...
    def access(
        self, *, phase: str, allowed_splits: Sequence[str]
    ) -> Tuple[Tuple[Stage3Record, ...], Dict[str, ImageObservation]]:
        allowed = tuple(str(item) for item in allowed_splits)
        allowed_set = set(allowed)
        selected = tuple(
            record for record in self.records if record.split in allowed_set
        )
        accessed_splits = tuple(sorted({record.split for record in selected}))
        violations = tuple(
            split for split in accessed_splits if split not in allowed_set
        )
        audit = {
            "phase": phase,
        ...
        if any(
            split.startswith("final_")
            for split in accessed_splits
            if phase in {"select_architecture", "calibrate"}
        ):
            audit["violations"] = list(audit["violations"]) + [
                "forbidden_final_split_access"
            ]
        self.phase_audits.append(audit)
        if audit["violations"]:
            raise VPMValidationError(
                f"phase access violation for {phase}: {audit['violations']}"
            )
        return (
            selected,
            {
                record.observation_id: self.observations[record.observation_id]
            ...
            for cooldown in (0, 1):
                ...
                rows.append(
                    (
                        ...
                        ImageObservation(pixels, source_id=row_id),
                    )
                )
    return tuple(rows)


def _sample_nonprototype_rows(
...
def _sampled_rows(
    ...
    for record in benchmark.record_by_split("prototype"):
        ...
        rows.append(
            (record.row_id, record.action_id, benchmark.prototypes[record.row_id][3])
        )
    return tuple(rows)
...
def _generator_identity(benchmark: Stage3Benchmark) -> Dict[str, Any]:
    ...
    final_descriptors = [
        ...
        if record.split.startswith("final_")
        or record.split == "information_theoretic_control"
    ]
    identity = {
        ...
        "diagnostic_development_row_ids": sorted(
            {
                record.row_id
        ...
        "architecture_selection_row_ids": sorted(
            {
                record.row_id
                for record in benchmark.records
                if record.split.startswith("architecture_selection_")
                and record.row_id is not None
            }
        ),
        "calibration_row_ids": sorted(
            {
                record.row_id
                for record in benchmark.records
                if record.split.endswith("calibration") and record.row_id is not None
            }
        ),
...
def _generator_identity_v2(benchmark: Stage3Benchmark) -> Dict[str, Any]:
    ...
    source_files = [
        REPO_ROOT
        / "examples"
        / "arcade_visual_video_discriminative_evidence_benchmark.py",
    ...
    file_digests = {
        str(path.relative_to(REPO_ROOT)).replace("\\", "/"): _file_blob_digest(path)
    ...
    identity = {
        ...
        "evaluation_sample_row_ids": [
            row_id for row_id, _action_id, _observation in sample
        ],
    }
    identity["generator_identity_digest"] = _sha256(identity)
    return identity
...
def _generator_identity_v3(benchmark: Stage3Benchmark) -> Dict[str, Any]:
    ...
    source_files = [
        ...
        / "docs"
        / "research"
        / "video-stage-three-v3-joint-evidence-amendment.md",
        REPO_ROOT
        / "docs"
        / "research"
        / "video-stage-three-v3-operational-contract.md",
    ]
    file_digests = {
        str(path.relative_to(REPO_ROOT)).replace("\\", "/"): _file_blob_digest(path)
        for path in source_files
...
def _nearest_same_action(
    row_id: str, action_id: str, rows: Sequence[Tuple[str, str, ImageObservation]]
) -> Tuple[str, ImageObservation]:
...
def _nearest_conflicting_action(
    row_id: str, action_id: str, rows: Sequence[Tuple[str, str, ImageObservation]]
) -> Tuple[str, ImageObservation]:
    for other_row_id, other_action_id, observation in rows:
        if other_row_id != row_id and other_action_id != action_id:
...
def _region_manifest() -> Tuple[DiscriminativeRegionSpec, ...]:
    ...
    return (
        DiscriminativeRegionSpec(
            ...
            metadata={
                "semantic_role": "target evidence",
                "reason_for_inclusion": "target alignment and ambiguity",
            },
        ),
        DiscriminativeRegionSpec(
            ...
            registration_config=registration,
            metadata={
                "semantic_role": "cooldown evidence",
                "reason_for_inclusion": "action-critical evidence",
            },
        ),
        DiscriminativeRegionSpec(
            region_id="tank_band",
            ...
            registration_config=registration,
            metadata={
                "semantic_role": "tank evidence",
                "reason_for_inclusion": "pose and local displacement evidence",
            },
        ),
    )
...
def _split_family_definitions() -> Mapping[str, Sequence[Tuple[str, str]]]:
    return {
        ...
        "diagnostic_development": (
            (
                "development_photometric",
                "mild photometric variation for mask stability",
            ),
        ...
        "architecture_selection_benign": (
            ...
            (
                "selection_translation_photometric",
                "translation plus photometric change",
            ),
        ...
        "architecture_selection_negative": (
            ...
            (
                "selection_negative_action_critical_removed",
                "action-critical evidence removal",
            ),
            (
                "selection_negative_same_action_wrong_row",
                "same-action wrong-row construction",
            ),
            (
                "selection_negative_conflicting_action",
                "conflicting-action near neighbour",
            ),
            (
                "selection_negative_compositional_invalid",
                "compositional invalid observation",
            ),
        ...
        "rejection_calibration": (
            (
                "calibration_negative_critical_removed",
                "fresh critical evidence removal",
            ),
            (
                "calibration_negative_action_critical_removed",
                "fresh action-critical evidence removal",
            ),
            (
                "calibration_negative_same_action_wrong_row",
                "fresh same-action wrong-row",
            ),
            (
                "calibration_negative_conflicting_action",
                "fresh conflicting-action neighbour",
            ),
            (
                "calibration_negative_compositional_invalid",
                "fresh compositional invalid",
            ),
        ...
        "final_distinguishable_negative": (
            ...
            (
                "final_negative_conflicting_action",
                "frozen future conflicting-action neighbour",
            ),
            (
                "final_negative_compositional_invalid",
                "frozen future compositional invalid",
            ),
        ),
        "information_theoretic_control": (
            (
                "final_information_control",
                "frozen future information-theoretic controls",
            ),
        ),
    }
...
def _make_observation_id_for_config(
    config: Stage3VersionConfig, split: str, family_id: str, row_id: str, variant: int
) -> str:
...
def _build_stage3_benchmark(*, materialize_final: bool = False) -> Stage3Benchmark:
    ...
    for row_id, action_id, observation in sampled_rows:
        ...
        conflicting_row_id, conflicting_observation = _nearest_conflicting_action(
            row_id, action_id, rows
        )
        transformations = {
            "prototype_clean": lambda base=observation.pixels: base,
            "development_photometric": lambda base=observation.pixels: _scale_intensity(
                base, numerator=96, offset=3
            ),
            "development_shift": lambda base=observation.pixels: _translate(
                base, dx=1, dy=0
            ),
            "selection_exact": lambda base=observation.pixels: base,
            "selection_translation_x": lambda base=observation.pixels: _translate(
                base, dx=1, dy=0
            ),
            "selection_translation_y": lambda base=observation.pixels: _translate(
                base, dx=0, dy=1
            ),
            "selection_translation_xy": lambda base=observation.pixels: _translate(
                base, dx=1, dy=1
            ),
            "selection_photometric": lambda base=observation.pixels: _scale_intensity(
                base, numerator=92, offset=4
            ),
            "selection_noncritical_occlusion": lambda base=observation.pixels: (
                _mask_box(base, top=0, left=0, height=2, width=3, value=90)
            ),
            "selection_critical_partial": lambda base=observation.pixels: _mask_box(
                base, top=7, left=25, height=1, width=2, value=0
            ),
            "selection_translation_photometric": lambda base=observation.pixels: (
                _scale_intensity(_translate(base, dx=1, dy=0), numerator=94, offset=2)
            ),
            "selection_translation_occlusion": lambda base=observation.pixels: (
                _mask_box(
                    _translate(base, dx=1, dy=0),
                    top=0,
                    left=0,
                    height=2,
                    width=3,
            ...
            "selection_same_action_ambiguous": lambda base=observation.pixels, same=same_observation.pixels: (
                _blend_region(base, same, top=0, left=0, height=6, width=28)
            ),
            "selection_mixed_action_ambiguous": lambda base=observation.pixels, other=conflicting_observation.pixels: (
                _blend_region(base, other, top=7, left=25, height=2, width=2)
            ),
            "selection_negative_critical_removed": lambda base=observation.pixels: (
                _mask_box(base, top=7, left=25, height=2, width=2, value=0)
            ),
            "selection_negative_action_critical_removed": lambda base=observation.pixels: (
                _mask_box(base, top=7, left=25, height=2, width=2, value=255)
            ),
            "selection_negative_same_action_wrong_row": lambda base=observation.pixels, same=same_observation.pixels: (
                _blend_region(base, same, top=0, left=0, height=6, width=28)
            ),
            "selection_negative_conflicting_action": lambda base=observation.pixels, other=conflicting_observation.pixels: (
                _blend_region(base, other, top=7, left=25, height=2, width=2)
            ),
            "selection_negative_compositional_invalid": lambda base=observation.pixels, same=same_observation.pixels, other=conflicting_observation.pixels: (
                _blend_region(
                    _blend_region(base, same, top=10, left=0, height=4, width=28),
                    other,
                    top=0,
                    left=0,
                    height=6,
            ...
            "selection_negative_conflicting_regions": lambda base=observation.pixels, same=same_observation.pixels, other=conflicting_observation.pixels: (
                _blend_region(
                    _blend_region(base, same, top=0, left=0, height=6, width=28),
                    other,
                    top=10,
            ...
            "selection_negative_out_of_bounds": lambda base=observation.pixels: (
                _translate(base, dx=4, dy=0)
            ),
            "calibration_exact": lambda base=observation.pixels: _scale_intensity(
                base, numerator=100, offset=1
            ),
            "calibration_translation_x": lambda base=observation.pixels: _translate(
                base, dx=-1, dy=0
            ),
            "calibration_translation_xy": lambda base=observation.pixels: _translate(
                base, dx=-1, dy=1
            ),
            "calibration_photometric": lambda base=observation.pixels: _scale_intensity(
                base, numerator=90, offset=6
            ),
            "calibration_occlusion": lambda base=observation.pixels: _mask_box(
                base, top=1, left=1, height=2, width=3, value=120
            ),
            "calibration_same_action_ambiguous": lambda base=observation.pixels, same=same_observation.pixels: (
                _blend_region(base, same, top=10, left=0, height=4, width=28)
            ),
            "calibration_mixed_action_ambiguous": lambda base=observation.pixels, other=conflicting_observation.pixels: (
                _blend_region(base, other, top=7, left=25, height=2, width=2)
            ),
            "calibration_negative_critical_removed": lambda base=observation.pixels: (
                _mask_box(base, top=7, left=25, height=2, width=2, value=0)
            ),
            "calibration_negative_action_critical_removed": lambda base=observation.pixels: (
                _mask_box(base, top=7, left=25, height=2, width=2, value=255)
            ),
            "calibration_negative_same_action_wrong_row": lambda base=observation.pixels, same=same_observation.pixels: (
                _blend_region(base, same, top=0, left=0, height=6, width=28)
            ),
            "calibration_negative_conflicting_action": lambda base=observation.pixels, other=conflicting_observation.pixels: (
                _blend_region(base, other, top=7, left=25, height=2, width=2)
            ),
            "calibration_negative_compositional_invalid": lambda base=observation.pixels, same=same_observation.pixels, other=conflicting_observation.pixels: (
                _blend_region(
                    _blend_region(base, same, top=10, left=0, height=4, width=28),
                    other,
                    top=0,
            ...
            "calibration_negative_conflicting_regions": lambda base=observation.pixels, same=same_observation.pixels, other=conflicting_observation.pixels: (
                _blend_region(
                    _blend_region(base, same, top=0, left=0, height=6, width=28),
                    other,
                    top=10,
            ...
            "calibration_negative_out_of_bounds": lambda base=observation.pixels: (
                _translate(base, dx=-4, dy=0)
            ),
        }
        for split, family_items in families.items():
            for family_id, description in family_items:
                ...
                if split == "prototype":
                    ...
                    pixels = (
                        transformations["development_photometric"]()
                        if family_id == "development_photometric"
                        else transformations["development_shift"]()
                    )
                    materialized = True
                    expected_disposition = "expected_accept"
                    ...
                    expected_disposition = (
                        "information_theoretic_control"
                        if split == "information_theoretic_control"
                        else (
                            "expected_accept"
                            if split == "final_benign"
                    ...
                    if materialized:
                        pixels = (
                            observation.pixels
                            if family_id in {"final_exact", "final_information_control"}
                            else _scale_intensity(
                                observation.pixels,
                                numerator=97,
                                offset=(len(family_id) % 7) + 1,
                            )
                        )
                    else:
                        pixels = None
                metadata = {
                    ...
                    "distinguishable_negative": split
                    in {
                        "architecture_selection_negative",
                        "rejection_calibration",
                        "final_distinguishable_negative",
                    },
                    "information_theoretic_control": split
                    == "information_theoretic_control",
                }
                if materialized:
    ...
    split_counts = {
        split: sum(record.split == split for record in records) for split in SPLIT_ROLES
    }
    return Stage3Benchmark(
        ...
        family_specs={
            family_id: {"description": description, "split": split}
            for split, items in families.items()
...
def _split_family_definitions_v2() -> Mapping[str, Sequence[Tuple[str, str]]]:
    ...
    families["prototype"] = (
        ("prototype_clean_v2", "canonical prototype observations"),
    )
    return families
...
def _build_stage3_benchmark_v2(*, materialize_final: bool = False) -> Stage3Benchmark:
    ...
    def _make_stage_record(
        *,
        ...
        observation_id = _make_observation_id_for_config(
            config, split, family_id, row_id, 0
        )
    ...
    for row_id, action_id, observation in rows:
        ...
        for split, family_items in families.items():
            if split in {
                ...
                "final_distinguishable_negative",
                "information_theoretic_control",
            } and row_id not in {item[0] for item in sampled_rows}:
                continue
            for family_id, description in family_items:
                if split == "prototype":
...
def _build_stage3_benchmark_v3(*, materialize_final: bool = False) -> Stage3Benchmark:
    ...
    for record in benchmark.records:
        ...
        metadata = {
            **_json_ready(record.metadata),
            "benchmark_version": config.benchmark_version,
        }
        if record.materialized:
            original = benchmark.observations[record.observation_id]
            image = ImageObservation(
                original.pixels, source_id=observation_id, metadata=metadata
            )
    ...
    split_counts = {
        split: sum(record.split == split for record in rebuilt_records)
...
def _split_manifest(benchmark: Stage3Benchmark) -> Dict[str, Any]:
    membership = {
        split: [record.observation_id for record in benchmark.record_by_split(split)]
        for split in SPLIT_ROLES
    }
    clip_membership = {
        split: sorted({record.clip_id for record in benchmark.record_by_split(split)})
        for split in SPLIT_ROLES
    }
    return {
...
def _development_observations(
    benchmark: Stage3Benchmark,
) -> Mapping[str, Tuple[ImageObservation, ...]]:
...
def _freeze_regions_and_masks(
    benchmark: Stage3Benchmark, *, output_dir: Path
) -> Dict[str, Any]:
    ...
    region_manifest = {
        ...
        "registration_contract_digest_set": sorted(
            {region.registration_config.digest for region in regions}
        ),
    }
    mask_manifest = {
        "mask_specs": [mask.spec.to_dict() for _row_id, mask in sorted(masks.items())],
        "mask_payload_digests": {
            row_id: mask.payload_digest for row_id, mask in sorted(masks.items())
        },
        "mask_spec_digest": discriminative_mask_digest(
            tuple(mask.spec for mask in masks.values())
        ),
        "prototype_digest": _sha256(
            {key: value[2] for key, value in sorted(benchmark.prototypes.items())}
        ),
        "development_digest": _sha256(
            {
                row_id: [item.raw_digest for item in values]
                for row_id, values in sorted(
                    _development_observations(benchmark).items()
                )
            }
        ),
...
def validate_prototype_and_development_closure(
    *,
    ...
    architecture_selection_rows = sorted(
        {
            record.row_id
            for record in benchmark.records
            if record.split.startswith("architecture_selection_")
    ...
    final_descriptor_rows = sorted(
        {
            record.row_id
            for record in benchmark.records
            if (
                record.split.startswith("final_")
                or record.split == "information_theoretic_control"
            )
            and record.row_id is not None
        }
    ...
    rows_with_nonzero_stability = sorted(
        row_id
        for row_id, mask in masks.items()
        if int(mask.spec.stable_pixel_count) > 0
    )
    rows_with_zero_stability = sorted(
        row_id
        for row_id, mask in masks.items()
        if int(mask.spec.stable_pixel_count) == 0
    )
    missing_prototype_manifest_rows = sorted(
        set(provider_prototype_rows) - set(prototype_split_rows)
    )
    missing_mask_rows = sorted(set(provider_prototype_rows) - set(mask_rows))
    missing_development_rows = sorted(
        set(provider_prototype_rows) - set(development_rows)
    )
    evaluated_rows_with_zero_stability = sorted(
        set(architecture_selection_rows) & set(rows_with_zero_stability)
    )
    closure_failures = []
...
def _mask_coverage_report(
    *, benchmark: Stage3Benchmark, masks: Mapping[str, Any], output_dir: Path
) -> Dict[str, Any]:
    ...
    for row_id in prototype_rows:
        ...
        effective_mass = (
            ...
            if mask is None
                ...
                mask.stable_weights.sum(dtype=np.float64)
                * mask.row_informative_weights.sum(dtype=np.float64)
            )
        )
        ...
        rows.append(
            {
                ...
                "development_observation_count": len(
                    development_by_row.get(row_id, ())
                ),
                "development_family_ids": sorted(development_by_row.get(row_id, ())),
                "informative_pixel_count": None
                if mask is None
                else int(mask.spec.informative_pixel_count),
                "action_conflict_pixel_count": None
                if mask is None
                else int(mask.spec.action_conflict_pixel_count),
                ...
                "appears_in_architecture_selection_expected_rows": row_id
    ...
    summary = {
        ...
        "nonzero_stability_masks": sum(
            1 for row in rows if (row["stable_pixel_count"] or 0) > 0
        ),
        "zero_stability_masks": sum(
            1 for row in rows if row["stable_pixel_count"] == 0
        ),
        "evaluated_rows_with_zero_stability": sum(
            1
            for row in rows
            if row["appears_in_architecture_selection_expected_rows"]
            and row["stable_pixel_count"] == 0
        ),
        "candidate_rows_with_zero_stability": sum(
            1 for row in rows if row["stable_pixel_count"] == 0
        ),
        "actions_affected": sorted(
            {row["action_id"] for row in rows if row["stable_pixel_count"] == 0}
        ),
...
def _prototype_manifest_v2(
    benchmark: Stage3Benchmark, *, generator_identity: Mapping[str, Any]
) -> Dict[str, Any]:
    rows = []
    prototype_records = [
        record
        ...
        rows.append(
            {
                ...
                "frame_id": record.frame_id,
                "clip_id": record.clip_id,
                "pixel_digest": benchmark.observations[
                    record.observation_id
                ].raw_digest,
                "generator_identity_digest": generator_identity[
                    "generator_identity_digest"
...
def _prototype_collision_atlas_v2(benchmark: Stage3Benchmark) -> Dict[str, Any]:
    ...
    for row_id, (prototype_observation_id, action_id, _digest, observation) in sorted(
        benchmark.prototypes.items()
    ):
        groups[observation.raw_digest].append(
            {
                "row_id": row_id,
                "action_id": action_id,
...
def _mask_closure_v2(
    benchmark: Stage3Benchmark, masks: Mapping[str, Any]
) -> Dict[str, Any]:
        ...
        rows.append(
            {
                ...
                "action_conflict_pixel_count": int(
                    mask.spec.action_conflict_pixel_count
                ),
    ...
    return {
        ...
        "masks_with_development_closure": sum(
            1 for row in rows if row["closure_status"] == "closed"
        ),
        "masks_with_nonzero_effective_evidence": effective_count,
        "remaining_zero_evidence_masks": sum(
            1 for row in rows if int(row["effective_informative_stable_mass"]) <= 0
        ),
        "rows": rows,
    ...
    *,
    ...
    return DiscriminativeEvidenceCalibration(
        ...
        maximum_critical_contradiction=float(
            values.get("maximum_critical_contradiction", 1.0)
        ),
        ...
        conflicting_action_separation=float(
            values.get("conflicting_action_separation", 0.0)
        ),
...
def _architecture_grid_definition() -> Dict[str, Any]:
    ...
    return {
        ...
        "C": {
            **common,
            "minimum_support": (0.0, 0.4),
            "maximum_contradiction": (0.5,),
            "maximum_critical_contradiction": (0.0, 0.25),
...
def _evaluate_provider(
    *,
    ...
    return _evaluate_ranked_candidates(
        ranked_by_observation={
            record.observation_id: provider._rank(observations[record.observation_id])
    ...
    for record in records:
        ...
        if record.expected_disposition == "expected_accept":
            ...
            if candidate_set.outcome == "exact_row_accepted":
                ...
                if (
                    record.row_id in candidate_set.rows
                    and len(candidate_set.rows) <= MAXIMUM_USEFUL_CANDIDATE_SET_SIZE
                ):
            ...
            if candidate_set.outcome == "exact_row_accepted":
                ...
                if (
                    candidate_set.exact_row_id is not None
                    ...
                    if (
                        next(
                            (
                                ...
                                if candidate.row_id == candidate_set.exact_row_id
                            ),
                            None,
                        )
                        != record.action_id
                    ...
                    if (
                        next(
                            (
                                candidate.aggregate_critical_contradiction
                                for candidate in ranked
                                if candidate.row_id == candidate_set.exact_row_id
                            ),
                            0.0,
            ...
            per_family[(record.family_id, candidate_set.outcome)] += 1
    exact_row_coverage = (
        0.0 if benign_total == 0 else exact_correct / float(benign_total)
    )
    exact_precision = (
        None if exact_accepted == 0 else exact_correct / float(exact_accepted)
    )
    return {
        ...
        "useful_candidate_set_recall": 0.0
        if benign_total == 0
        else useful_candidate_set_count / float(benign_total),
        ...
        "per_family_outcomes": {
            ...
            for (family, outcome), count in sorted(per_family.items())
        },
        "cache_hits": 0,
...
def _selection_rank_key(result: Mapping[str, Any]) -> Tuple[Any, ...]:
    ...
    return (
        ...
        0
        if (metrics["exact_correct_count"] + metrics["useful_candidate_set_count"]) > 0
        else 1,
        -float(metrics["exact_row_coverage"]),
        -int(metrics["useful_candidate_set_count"]),
        float(
            metrics["mean_set_size"] if metrics["mean_set_size"] is not None else 99.0
        ),
...
def _feasible_for_selection(result: Mapping[str, Any]) -> Tuple[bool, Tuple[str, ...]]:
    ...
    if (
        SELECTION_NEGATIVE_CANDIDATE_SET_SUPPORT_BLOCKS_FEASIBILITY
        and int(metrics["negative_candidate_set_support"]) != 0
    ):
        reasons.append("negative_candidate_set_support")
    if (
        int(metrics["exact_correct_count"]) + int(metrics["useful_candidate_set_count"])
        == 0
    ):
...
def _run_selection(output_dir: Path) -> Dict[str, Any]:
    ...
    benign_eval = tuple(
        ...
        if record.split == "architecture_selection_benign"
    )
    negative_eval = tuple(
        record
        for record in negative_records
        if record.split == "architecture_selection_negative"
    )
    grid_definition = _architecture_grid_definition()
    _write_json(
        output_dir / "architecture-grid-definition.json",
        {
            "definition": _json_ready(grid_definition),
            "digest": _sha256(grid_definition),
        },
    )
    results = []
    raw_benign = {
        architecture_id: _raw_ranked_candidates(
            architecture_id=architecture_id,
            benchmark=benchmark,
            freeze=freeze,
            records=benign_eval,
            observations=benign_observations,
        )
        ...
        for point in _grid_points(grid_definition[architecture_id]):
            ...
            combined_metrics = {
                **benign_metrics,
                ...
                "negative_candidate_set_support": negative_metrics[
                    "negative_candidate_set_support"
                ],
                "conflicting_action_exact_accepts": negative_metrics[
                    "conflicting_action_exact_accepts"
                ],
                "same_action_wrong_row_exact_accepts": negative_metrics[
                    "same_action_wrong_row_exact_accepts"
                ],
                "critical_contradiction_exact_accepts": negative_metrics[
                    "critical_contradiction_exact_accepts"
                ],
                "negative_per_family_outcomes": negative_metrics["per_family_outcomes"],
            }
            feasible, infeasible_reasons = _feasible_for_selection(
    ...
    best_by_architecture = {
        architecture_id: min(
            (
                ...
                if result["architecture_id"] == architecture_id
            ),
    ...
    d_gateway = {
        "conditions": {
            "a_nonzero_safe_utility": (
                a["combined_metrics"]["exact_correct_count"]
                + a["combined_metrics"]["useful_candidate_set_count"]
            )
            > 0,
    ...
    for architecture_id in ("B", "C"):
        ...
        if (
            candidate["combined_metrics"]["exact_correct_count"]
            + candidate["combined_metrics"]["useful_candidate_set_count"]
            > a["combined_metrics"]["exact_correct_count"]
            + a["combined_metrics"]["useful_candidate_set_count"]
        ):
            d_gateway["conditions"]["b_or_c_additional_recovery"] = True
            if (
                candidate["combined_metrics"]["exact_false_accepts"] == 0
                and candidate["combined_metrics"]["conflicting_action_exact_accepts"]
                == 0
                and candidate["combined_metrics"][
                    "critical_contradiction_exact_accepts"
                ]
                == 0
            ):
                d_gateway["conditions"]["added_recovery_zero_new_safety_failures"] = (
                    True
                )
            recovered_families = sorted(
                {
                    key.split(":")[0]
                    for key, value in candidate["combined_metrics"][
                        "per_family_outcomes"
                    ].items()
                    if key.endswith("exact_row_accepted")
                    or key.endswith("candidate_set_available")
                }
            )
            a_families = sorted(
                {
                    key.split(":")[0]
                    for key, value in a["combined_metrics"][
                        "per_family_outcomes"
                    ].items()
                    if key.endswith("exact_row_accepted")
                    or key.endswith("candidate_set_available")
            ...
            break
    d_gateway["eligible"] = all(
        bool(value) for value in d_gateway["conditions"].values()
    )
    _write_json(
        output_dir / "architecture-d-gateway.json",
        {**d_gateway, "digest": _sha256(d_gateway)},
    )
    if d_gateway["eligible"]:
        raw_benign["D"] = _raw_ranked_candidates(
            architecture_id="D",
            benchmark=benchmark,
            freeze=freeze,
            records=benign_eval,
        ...
        raw_negative["D"] = _raw_ranked_candidates(
            architecture_id="D",
            benchmark=benchmark,
            freeze=freeze,
            records=negative_eval,
    ...
    _write_csv(
        output_dir / "architecture-grid.csv",
        [
            {
    ...
    if not feasible_results:
        selection = {
            ...
            "architecture_selection_split_digest": _sha256(
                {
                    "benign": [record.observation_id for record in benign_eval],
                    "negative": [record.observation_id for record in negative_eval],
                }
            ),
...
def _calibration_rank_key(result: Mapping[str, Any]) -> Tuple[Any, ...]:
    ...
    return (
        ...
        -(
            0.0
            if metrics["exact_accepted_precision"] is None
            else float(metrics["exact_accepted_precision"])
        ),
        float(
            metrics["mean_set_size"] if metrics["mean_set_size"] is not None else 99.0
        ),
...
def _run_calibrate(output_dir: Path) -> Dict[str, Any]:
    ...
    benign_eval = tuple(
        record for record in records if record.split == "benign_calibration"
    )
    rejection_eval = tuple(
        record for record in records if record.split == "rejection_calibration"
    )
    architecture_id = str(selection["selected_architecture"])
    grid_definition = _calibration_grid_definition(architecture_id)
    _write_json(
        output_dir / "calibration-grid-definition.json",
        {
            "architecture_id": architecture_id,
            "definition": _json_ready(grid_definition),
            "digest": _sha256(grid_definition),
    ...
    results = []
    raw_benign = _raw_ranked_candidates(
        architecture_id=architecture_id,
        benchmark=benchmark,
        freeze=freeze,
        records=benign_eval,
    ...
    raw_rejection = _raw_ranked_candidates(
        architecture_id=architecture_id,
        benchmark=benchmark,
        freeze=freeze,
    ...
    for point in _grid_points(grid_definition):
        ...
        metrics = {
            **benign_metrics,
            ...
            "negative_candidate_set_support": rejection_metrics[
                "negative_candidate_set_support"
            ],
            "conflicting_action_exact_accepts": rejection_metrics[
                "conflicting_action_exact_accepts"
            ],
            "same_action_wrong_row_exact_accepts": rejection_metrics[
                "same_action_wrong_row_exact_accepts"
            ],
            "critical_contradiction_exact_accepts": rejection_metrics[
                "critical_contradiction_exact_accepts"
            ],
    ...
    _write_csv(
        output_dir / "calibration-grid.csv",
        [
            {
                **result["calibration_values"],
                "architecture_id": architecture_id,
    ...
    if not feasible_results:
        artifact = {
            ...
            "split_digest": _sha256(
                {
                    "benign": [record.observation_id for record in benign_eval],
                    "rejection": [record.observation_id for record in rejection_eval],
                }
            ),
            "region_digest": freeze["region_manifest"]["region_spec_digest"],
            "mask_digest": freeze["mask_manifest"]["mask_spec_digest"],
...
def _freeze_final_identities(output_dir: Path) -> Dict[str, Any]:
    ...
    return {
        "benchmark_manifest_digest": _sha256(manifest),
...
def _verify_pre_final(output_dir: Path) -> Dict[str, Any]:
    ...
    architecture_grid_definition = _load_json(
        output_dir / "architecture-grid-definition.json"
    )
    ...
    calibration_grid_definition = (
        _load_json(output_dir / "calibration-grid-definition.json")
        if (output_dir / "calibration-grid-definition.json").exists()
        else None
    )
    selected_operating_point = (
        _load_json(output_dir / "selected-operating-point.json")
        if (output_dir / "selected-operating-point.json").exists()
        else None
    )
    ...
    for audit in phase_audits:
        if any(
            split.startswith("final_") for split in audit["accessed_splits"]
        ) and audit["phase"] in {"select_architecture", "calibrate"}:
            violations.append("final_split_accessed_early")
    forbidden_final_outputs = [
        name
        for name in ("final-metrics.json", "final-evaluation.json")
    ...
    payload = {
        ...
        "calibration_grid_digest": None
        if calibration_grid_definition is None
        else calibration_grid_definition["digest"],
        "selected_operating_point_digest": None
        if selected_operating_point is None
        else _sha256(selected_operating_point),
    }
    if violations:
...
def _audit_exact_frames(*, output_dir: Path) -> Dict[str, Any]:
    ...
    exact_records = tuple(
        record for record in records if record.family_id == "selection_exact"
    )
    rows = []
    summary: Dict[str, Any] = {
        "architectures": {},
        "exact_observation_count": len(exact_records),
    }
    for architecture_id in ("A", "B", "C"):
        ...
        for record in exact_records:
            ...
            expected = next(
                (
                    candidate
                    for candidate in ranked
                    if candidate.row_id == record.row_id
                ),
                None,
            )
            winner = ranked[0]
            ...
            if expected is not None and winner.row_id == record.row_id:
                top1 += 1
            rejection_reasons[
                candidate_set.rejection_reason or candidate_set.outcome
            ] += 1
            rows.append(
                {
                    "observation_id": record.observation_id,
                    ...
                    "expected_mask_stability_count": None
                    if expected is None
                    else int(freeze["masks"][record.row_id].spec.stable_pixel_count),
                    "expected_informative_count": None
                    if expected is None
                    else int(
                        freeze["masks"][record.row_id].spec.informative_pixel_count
                    ),
                    "expected_row_rank": None
                    if expected is None
                    else next(
                        (
                            index
                    ...
                    "expected_row_strength": None
                    if expected is None
                    else float(expected.candidate_strength),
                    "available_mass": None
                    if expected is None
                    else float(expected.available_informative_mass),
                    "available_fraction": None
                    if expected is None
                    else float(expected.available_informative_fraction),
                    "support": None
                    if expected is None
                    else float(expected.aggregate_support),
                    "contradiction": None
                    if expected is None
                    else float(expected.aggregate_contradiction),
                    "critical_contradiction": None
                    if expected is None
                    else float(expected.aggregate_critical_contradiction),
                    "supporting_regions": None
                    if expected is None
                    else int(expected.supporting_region_count),
                    ...
                    "winner_margin": None
                    if expected is None
                    else expected.exact_winner_margin,
                    "conflicting_action_separation": None
                    if expected is None
                    else expected.conflicting_action_separation,
                    "expected_row_candidate_eligible": None
                    if expected is None
                    else bool(expected.eligible_for_candidate_set),
                    "expected_row_exact_eligible": None
                    if expected is None
                    else bool(expected.eligible_for_exact),
                    "candidate_ineligibility_reasons": []
                    if expected is None
                    else list(expected.ineligibility_reasons),
...
def _classify_v1(
    *,
    ...
    development_failure = bool(
        closure["missing_development_rows"]
        or closure["evaluated_rows_with_zero_stability"]
    )
    wiring_failure = all(
        exact_summary["architectures"][architecture_id][
            "expected_rows_with_nonzero_evidence"
        ]
        == 0
        for architecture_id in ("A", "B", "C")
    )
    if generator_mismatch:
        evidence.append(
            "Current committed code does not regenerate the frozen v1 artifact set semantically."
        )
    if prototype_failure:
        evidence.append(
            "The provider prototype universe is larger than the prototype split manifest."
        )
    if development_failure:
        evidence.append(
            "Architecture-selection rows lack development-backed stable evidence."
        )
    if wiring_failure:
        evidence.append(
            "All audited exact frames lose expected-row evidence before the architecture-specific gates are meaningfully exercised."
        )
    if generator_mismatch and (
        prototype_failure or development_failure or wiring_failure
    ):
        ...
        evidence.append(
            "Committed code regenerates v1, prototype closure is complete, and exact rows retain evidence."
        )
    return {
        "ruling": label,
        "evidence": evidence,
        "affected_artifacts": [
            item["artifact"]
            for item in regeneration["artifacts"]
            if item["status"]
            in {"semantic_mismatch", "missing_artifact", "unexpected_artifact"}
        ],
...
def _exact_sanity_v2(
    *,
    benchmark: Stage3Benchmark,
    freeze: Mapping[str, Any],
    collision_atlas: Mapping[str, Any],
    output_dir: Path,
) -> Dict[str, Any]:
    collision_by_row = {}
    ...
    for architecture_id in ("A", "B", "C"):
        ...
        for record in exact_records:
            ranked = tuple(
                evaluate_candidate_eligibility(
                    candidate=candidate,
                    ranked_candidates=raw[record.observation_id],
                    calibration=provider._calibration,
                )
            ...
            if [candidate.to_dict() for candidate in raw[record.observation_id]] != [
                candidate.to_dict() for candidate in direct_ranked
            ]:
                sanity_valid = False
            collision = collision_by_row[record.row_id]
            expected = next(
                candidate for candidate in ranked if candidate.row_id == record.row_id
            )
            top_strength = ranked[0].candidate_strength
            sorted(
                candidate.row_id
                for candidate in ranked
                if abs(candidate.candidate_strength - top_strength) <= 1e-12
            )
            if collision["collision_class"] == "unique_visual_row":
                if (
                    expected.available_informative_mass <= 0
                    or ranked[0].row_id != record.row_id
                    or not expected.eligible_for_candidate_set
                ):
                    sanity_valid = False
                else:
                    unique_top1 += 1
            elif (
                collision["collision_class"] == "conflicting_action_visual_alias"
                and candidate_set.outcome == "exact_row_accepted"
            ):
                sanity_valid = False
            rows.append(
                {
                    ...
                    "expected_row_available_mass": float(
                        expected.available_informative_mass
                    ),
                    "expected_row_available_fraction": float(
                        expected.available_informative_fraction
                    ),
                    "expected_row_support": float(expected.aggregate_support),
                    "expected_row_contradiction": float(
                        expected.aggregate_contradiction
                    ),
                    "expected_row_critical_contradiction": float(
                        expected.aggregate_critical_contradiction
                    ),
                    "expected_row_supporting_regions": int(
                        expected.supporting_region_count
                    ),
                    ...
                    "expected_row_candidate_eligible": bool(
                        expected.eligible_for_candidate_set
                    ),
...
def _stage3_v2_preselection_valid(
    closure: Mapping[str, Any], exact_summary: Mapping[str, Any]
) -> bool:
    return (
        bool(closure["closure_valid"])
        and bool(exact_summary["sanity_valid"])
        and bool(exact_summary["direct_provider_equivalence"])
    )


def _write_stage3_v2_docs(
    output_dir: Path,
    *,
...
def _freeze_benchmark_v2_into(output_dir: Path) -> Dict[str, Any]:
    ...
    prototype_manifest = _prototype_manifest_v2(
        benchmark, generator_identity=generator_identity
    )
    development_manifest = _development_manifest_v2(
        benchmark, generator_identity=generator_identity
    )
    evaluation_sample = _evaluation_sample_manifest_v2(benchmark)
    freeze = _freeze_regions_and_masks(benchmark, output_dir=output_dir)
    prototype_rows = sorted(benchmark.prototypes)
    prototype_manifest_rows = sorted(
        row["row_id"] for row in prototype_manifest["rows"]
    )
    mask_rows = sorted(freeze["masks"])
    if prototype_rows != prototype_manifest_rows or prototype_rows != mask_rows:
        raise VPMValidationError(
            "provider prototype, prototype manifest, and mask rows must match exactly"
        )
    ...
    _write_json(
        output_dir / "mask-closure-summary.json",
        {key: value for key, value in closure.items() if key != "rows"},
    )
    exact_summary = _exact_sanity_v2(
        benchmark=benchmark,
        freeze=freeze,
    ...
    _write_json(
        output_dir / "architecture-grid-definition.json",
        {
            "definition": _json_ready(_architecture_grid_definition()),
            "digest": _sha256(_architecture_grid_definition()),
            "amendment_parent": V2_AMENDMENT_COMMIT,
        },
    )
    _write_json(output_dir / "phase-access-audits.json", benchmark.phase_audits)
    _write_stage3_v2_docs(
        output_dir,
        generator_identity=generator_identity,
...
def run_freeze_benchmark_v2(output_dir: Path) -> Dict[str, Any]:
    with tempfile.TemporaryDirectory(
        prefix="stage3-v2-freeze-", dir=str(REPO_ROOT)
    ) as tmp:
    ...
    return {
        ...
        "generator_identity_digest": result["generator_identity"][
            "generator_identity_digest"
        ],
    }
...
def _run_audit_pre_final_v1(output_dir: Path) -> Dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="stage3-v1-audit-", dir=str(REPO_ROOT)
    ) as tmp:
        temp_output = Path(tmp)
        ...
        closure = validate_prototype_and_development_closure(
            benchmark=benchmark, masks=freeze["masks"]
        )
        mask_coverage = _mask_coverage_report(
            benchmark=benchmark, masks=freeze["masks"], output_dir=audit_dir
        )
    ...
    report = {
        ...
        "committed_manifest_sample_size": _load_json(
            output_dir / "benchmark-manifest.json"
        )["metadata"]["nonprototype_row_sample_size"],
        "regeneration_audit": regeneration,
        "prototype_and_development_closure": closure,
        "mask_coverage_summary": {
            key: value for key, value in mask_coverage.items() if key != "rows"
        },
        "exact_frame_summary": exact_summary,
    }
    ruling = _classify_v1(
        regeneration=regeneration, closure=closure, exact_summary=exact_summary
    )
...
def _run_evaluate(output_dir: Path) -> Dict[str, Any]:
    ...
    if ruling is None:
        raise SystemExit(
            "Stage 3 v1 evaluation is blocked until measurement-audit/v1-ruling.json exists."
        )
    if ruling["ruling"] != "valid_no_safe_architecture":
        raise SystemExit(
            "Stage 3 v1 evaluation is blocked because the frozen measurement failed the integrity audit."
        )
...
def run_verify_v2_benchmark(output_dir: Path) -> Dict[str, Any]:
    with tempfile.TemporaryDirectory(
        prefix="stage3-v2-verify-", dir=str(REPO_ROOT)
    ) as tmp:
        temp_output = Path(tmp)
        _freeze_benchmark_v2_into(temp_output)
        artifact_names = sorted(
            (
                {path.name for path in temp_output.iterdir() if path.is_file()}
                | (
        ...
        comparison = (
            _artifact_comparison(output_dir, temp_output, artifact_names=artifact_names)
            if output_dir.exists()
            else {"semantic_match": False, "artifacts": []}
        )
    payload = {
        "mode": "verify-v2-benchmark",
...
def run_select_architecture_v2(output_dir: Path) -> Dict[str, Any]:
    ...
    if not _stage3_v2_preselection_valid(closure, exact_summary):
        artifact = {
            "selection_status": "invalid_architecture_measurement",
            "selected_architecture": None,
            "selected_calibration_digest": None,
        }
...
def _run_selection_v2(
    *, output_dir: Path, benchmark: Stage3Benchmark, freeze: Mapping[str, Any]
) -> Dict[str, Any]:
    benign_records, benign_observations = benchmark.access(
        phase="select_architecture",
        allowed_splits=(
...
def run_calibrate_v2(output_dir: Path) -> Dict[str, Any]:
    ...
    if selection["selection_status"] == "invalid_architecture_measurement":
        artifact = {
            "selection_status": "invalid_calibration_measurement",
            "selected_architecture": None,
            "selected_calibration_digest": None,
        }
        _write_json(output_dir / "selected-operating-point.json", artifact)
    ...
    if selection["selection_status"] != "selected_architecture":
        artifact = {
            "selection_status": "not_run_no_selected_architecture",
            "selected_architecture": None,
            "selected_calibration_digest": None,
        }
        _write_json(output_dir / "selected-operating-point.json", artifact)
...
def run_verify_pre_final_v2(output_dir: Path) -> Dict[str, Any]:
    with tempfile.TemporaryDirectory(
        prefix="stage3-v2-prefinal-", dir=str(REPO_ROOT)
    ) as tmp:
        temp_output = Path(tmp)
        _freeze_benchmark_v2_into(temp_output)
        ...
        comparison = _artifact_comparison(
            output_dir, temp_output, artifact_names=artifact_names
        )
    payload = {
...
def _strict_zero_margin_exact_eligibility(
    ...
    *,
    ...
    if not candidate.eligible_for_candidate_set:
        return False
    if (
        candidate.candidate_strength + zde.REGISTRATION_DISTANCE_TIE_EPSILON
    ...
    if candidate.exact_winner_margin is None:
        return False
    required = max(
        float(calibration.exact_winner_margin),
        float(zde.REGISTRATION_DISTANCE_TIE_EPSILON),
    )
...
def _canonical_rankings_for_architecture(
    *,
    ...
    for row_id, (_prototype_observation_id, _action_id, _digest, observation) in sorted(
        benchmark.prototypes.items()
    ):
        ...
        by_observation[row_id] = tuple(
            evaluate_candidate_eligibility(
                candidate=candidate, ranked_candidates=ranked, calibration=calibration
            )
            for candidate in ranked
        )
...
def _candidate_joint_fit(
    *,
    ...
    distance = (
        np.abs(
            observation_pixels.astype(np.float32) - candidate_pixels.astype(np.float32)
        )
        / 255.0
    )
...
def _minimum_cover_size(
    cover_sets: Sequence[set[int]], universe_size: int
) -> Optional[int]:
...
def _run_representation_audit_v2(output_dir: Path) -> Dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="stage3-v2-representation-audit-", dir=str(REPO_ROOT)
    ) as tmp:
        temp_output = Path(tmp)
    ...
    benchmark = _build_stage3_benchmark_v2(materialize_final=False)
    with tempfile.TemporaryDirectory(
        prefix="stage3-v2-representation-freeze-", dir=str(REPO_ROOT)
    ) as tmp:
    ...
    for architecture_id in ("A", "B", "C"):
        ...
        for row_id, ranked in sorted(ranked_by_architecture[architecture_id].items()):
            ...
            expected = next(
                candidate for candidate in ranked if candidate.row_id == row_id
            )
            tie_size = sum(
                abs(candidate.candidate_strength - ranked[0].candidate_strength)
                <= zde.REGISTRATION_DISTANCE_TIE_EPSILON
                for candidate in ranked
            )
            ...
            top1 += int(rank == 1)
            zero_strength_candidates += sum(
                abs(candidate.candidate_strength)
                <= zde.REGISTRATION_DISTANCE_TIE_EPSILON
                for candidate in ranked
            )
            if (
                abs(ranked[0].candidate_strength)
                <= zde.REGISTRATION_DISTANCE_TIE_EPSILON
            ):
                zero_strength_observations.add((architecture_id, row_id))
            strict_exact = _strict_zero_margin_exact_eligibility(
                expected if rank == 1 else ranked[0],
                calibration=base_calibrations[architecture_id],
            )
            candidate_set = build_discriminative_candidate_set(
                ranked_candidates=ranked,
            ...
            accepted_row = (
                candidate_set.rows[0]
                if candidate_set.outcome == "exact_row_accepted" and candidate_set.rows
                else None
            )
            if accepted_row is not None and (
                winner_selected_by == "lexical_tie" or not strict_exact
            ):
                incorrect_exact += 1
                incorrect_lexical_exact_accepts += 1
            state_key = "|".join(
                f"{key}={expected.observation_digest and benchmark.prototypes[row_id][3].metadata.get(key)}"
                for key in ("tank", "target", "cooldown")
            )
            ...
            canonical_rows.append(
                {
                    ...
                    "expected_available_mass": float(
                        expected.available_informative_mass
                    ),
                    "expected_available_fraction": float(
                        expected.available_informative_fraction
                    ),
                    "expected_support": float(expected.aggregate_support),
                    "expected_contradiction": float(expected.aggregate_contradiction),
                    "expected_critical_contradiction": float(
                        expected.aggregate_critical_contradiction
                    ),
                    "expected_supporting_region_count": int(
                        expected.supporting_region_count
                    ),
    ...
    self_summary = {
        ...
        "per_action_top1": {
            arch: dict(sorted(counter.items()))
            for arch, counter in sorted(per_action.items())
        },
        "per_state_factor_top1": {
            arch: dict(sorted(counter.items()))
            for arch, counter in sorted(per_factor.items())
        },
    }
    ...
    for row_id, mask in sorted(freeze["masks"].items()):
        ...
        candidate_pixels = zde._coerce_observation_pixels(
            benchmark.prototypes[row_id][3]
        )
        competitors = [
            (other_row, zde._coerce_observation_pixels(proto[3]), proto[1])
            for other_row, proto in sorted(benchmark.prototypes.items())
            if other_row != row_id
        ]
        ...
        rows_with_positive_separation += int(
            np.any(arrays["positive_row_stable_separation"] > 0.0)
        )
        diffs = np.stack(
            [
                np.abs(candidate_pixels.astype(np.int16) - other.astype(np.int16))
                for _other_row, other, _action in competitors
            ],
            axis=0,
        )
        ...
        conflict_diffs = np.stack(
            [
                np.abs(candidate_pixels.astype(np.int16) - other.astype(np.int16))
                for _other_row, other, action in competitors
                if action != mask.spec.action_id
            ],
            axis=0,
        )
        nearest_conflict = (
            conflict_diffs.min(axis=0)
            if len(conflict_diffs)
            else np.full(candidate_pixels.shape, 255)
        )
        mask_rows.append(
            {
                "row_id": row_id,
                ...
                "row_informative_stable_count": int(
                    np.count_nonzero(row_info & stable)
                ),
                "positive_separation_weight_count": int(np.count_nonzero(positive_sep)),
                "row_informative_stable_positive_separation_count": int(
                    np.count_nonzero(row_info & stable & positive_sep)
                ),
                "action_conflict_count": int(
                    np.count_nonzero(arrays["action_conflict"] > 0.0)
                ),
                "positive_action_conflict_separation_count": int(
                    np.count_nonzero(
                        arrays["positive_action_conflict_separation"] > 0.0
                    )
                ),
                "contradiction_pixels_informative_but_zero_weight": int(
                    np.count_nonzero(row_info & ~positive_sep)
                ),
                ...
                "nearest_conflicting_action_difference_digest": _sha256(
                    nearest_conflict.tolist()
                ),
            }
        )
    ...
    mask_summary = {
        ...
        "informative_zeroed_fraction": 0.0
        if informative_pixels == 0
        else float(zeroed_pixels / informative_pixels),
        "max_difference_informative_min_difference_zero_contradiction_rows": sum(
            int(row["contradiction_pixels_informative_but_zero_weight"] > 0)
    ...
    direct_a_top1 = 0
    j_top1 = Counter()
    for row_id, (_obs_id, action_id, _digest, observation) in sorted(
        benchmark.prototypes.items()
    ):
        obs_pixels = zde._coerce_observation_pixels(observation)
        candidate_pixels = {
            candidate_row: zde._coerce_observation_pixels(proto[3])
            for candidate_row, proto in sorted(benchmark.prototypes.items())
        }
        full_frame_unique = (
            sum(
                int(np.array_equal(obs_pixels, pixels))
                for pixels in candidate_pixels.values()
            )
            == 1
        )
        region_uniques = []
        ...
        for region in freeze["regions"]:
            ...
            tuples = [
                _region_crop(pixels, region).tobytes()
                for pixels in candidate_pixels.values()
            ]
            ...
            if not single_pixel_unique:
                for idx, value in enumerate(region_obs.reshape(-1)):
                    column = [
                        _region_crop(pixels, region).reshape(-1)[idx]
                        for pixels in candidate_pixels.values()
                    ]
                        ...
                        if (
                            sum(
                                int(
                                    (
                                        _region_crop(pixels, region).reshape(-1)[idx],
                                        _region_crop(pixels, region).reshape(-1)[jdx],
                                    )
                                    == pair
                                )
                                for pixels in candidate_pixels.values()
        ...
        for candidate_row, pixels in candidate_pixels.items():
            ...
            for region in freeze["regions"]:
                reg = zde.extract_candidate_region_evidence(
                    ...
                    competing_prototypes={
                        key: value
                        for key, value in benchmark.prototypes.items()
                        if key != candidate_row
                    },
                    region=region,
                )
                direct_region_similarity.append(
                    (
                        1.0
                        - float(
                            reg.registration_dx == 0
                            and reg.registration_dy == 0
                            and 0.0
                            or 0.0
                        )
                    )
                ...
                region_vector.append(
                    _candidate_joint_fit(
                        ...
                        weight_mask=_region_crop(
                            _effective_mask_arrays(freeze["masks"][candidate_row])[
                                "positive_row_stable"
                            ],
                            region,
                        ),
            ...
            direct_scores.append(
                (candidate_row, float(np.mean(direct_region_similarity)))
            )
            j1 = _candidate_joint_fit(
                candidate_pixels=pixels,
                observation_pixels=obs_pixels,
                weight_mask=_effective_mask_arrays(freeze["masks"][candidate_row])[
                    "positive_row_stable"
                ],
            )
            ...
            for other_row, other_pixels in candidate_pixels.items():
                if other_row == candidate_row:
                    continue
                pair_mask = (
                    _effective_mask_arrays(freeze["masks"][candidate_row])["stable"]
                    > 0.0
                ) & (
                    np.abs(pixels.astype(np.int16) - other_pixels.astype(np.int16))
                    > int(freeze["masks"][candidate_row].spec.intensity_tolerance)
                )
                pair_weight = pair_mask.astype(np.float32)
                fit_c = _candidate_joint_fit(
                    candidate_pixels=pixels,
                    observation_pixels=obs_pixels,
                    weight_mask=pair_weight,
                )
                fit_o = _candidate_joint_fit(
                    candidate_pixels=other_pixels,
                    observation_pixels=obs_pixels,
                    weight_mask=pair_weight,
                )
            ...
            j4_scores.append(
                (
                    ...
                    float(
                        sum(
                            (index + 1) * value
                            for index, value in enumerate(region_vector)
                        )
                    ),
                )
            )
        ...
        for label, scores in (
            ...
            j_top1[label] += int(
                sorted(scores, key=lambda item: (-item[1], item[0]))[0][0] == row_id
            )
        ...
        distributed.append(
            {
                ...
                "distributed_competitor_cover": distributed_cover is not None
                and distributed_cover > 1,
            }
        )
        ...
        arch_a_rows.append(
            {
                ...
                "implemented_expected_rank": next(
                    index
                    for index, candidate in enumerate(ranked_a, start=1)
                    if candidate.row_id == row_id
                ),
                "implemented_expected_strength": float(
                    next(
                        candidate
    ...
    compositional_summary = {
        ...
        "individual_pixel_unique_row_count": sum(
            int(row["individual_pixel_unique"]) for row in comp_rows
        ),
        "region_joint_unique_row_count": sum(
            int(row["region_joint_unique"]) for row in comp_rows
        ),
        "cross_region_joint_unique_row_count": sum(
            int(row["cross_region_joint_unique"]) for row in comp_rows
        ),
        "full_frame_unique_row_count": sum(
            int(row["full_frame_unique"]) for row in comp_rows
        ),
    }
    _write_json(
        audit_dir / "compositional-uniqueness.json",
        {"rows": comp_rows, "summary": compositional_summary},
    )
    distributed_summary = {
        "row_count": len(distributed),
        "distributed_competitor_cover_row_count": sum(
            int(row["distributed_competitor_cover"]) for row in distributed
        ),
        "rows": distributed,
    }
    ...
    a_ruling = (
        "architecture_a_implementation_defect"
        if implemented_a_top1 != direct_a_top1
        else "architecture_a_conformant"
    )
    a_summary = {
        "ruling": a_ruling,
    ...
    exact_tie = {
        "ruling": "exact_tie_implementation_defect"
        if incorrect_lexical_exact_accepts > 0
        else "exact_tie_logic_valid",
        "incorrect_lexical_exact_accepts": incorrect_lexical_exact_accepts,
        "strict_superiority_required": True,
    }
    _write_json(audit_dir / "exact-tie-audit.json", exact_tie)
    ruling = {
        "primary_ruling": "multiple_representation_failures"
        if (
            a_ruling == "architecture_a_implementation_defect"
            and exact_tie["ruling"] == "exact_tie_implementation_defect"
        )
        else "pointwise_representation_limitation",
        "mask_separation_ruling": "implementation_conformance_defect"
        if mask_summary["informative_pixels_zeroed_by_nearest_competitor_rule"] > 0
        else "valid_frozen_provider_failure",
    ...
    payload = {
        ...
        "distributed_competitor_cover_row_count": distributed_summary[
            "distributed_competitor_cover_row_count"
        ],
    ...
    if mutated:
        raise SystemExit(
            json.dumps(
                _json_ready({**payload, "failure": "v2_root_artifacts_modified"}),
                indent=2,
                sort_keys=True,
            )
        )
    if (
        a_ruling != "architecture_a_conformant"
        or exact_tie["ruling"] != "exact_tie_logic_valid"
        or ruling["primary_ruling"] != "valid_frozen_provider_failure"
    ):
        raise SystemExit(json.dumps(_json_ready(payload), indent=2, sort_keys=True))
    return payload
...
def run_evaluate_v2(output_dir: Path) -> Dict[str, Any]:
    ...
    verification = (
        _load_json(output_dir / "pre-final-verification.json")
        if (output_dir / "pre-final-verification.json").exists()
        else None
    )
    if selection.get("selection_status") != "selected_architecture":
        raise SystemExit(
            "V2 evaluation is blocked because no architecture has been validly selected."
        )
    if (
        calibration is None
        or calibration.get("selection_status") != "selected_operating_point"
    ):
        raise SystemExit(
            "V2 evaluation is blocked because no valid operating point has been selected."
        )
    if verification is None or not verification.get("verified"):
        raise SystemExit(
            "V2 evaluation is blocked because pre-final verification has not passed."
        )
    raise SystemExit(
        "V5 and final evaluation remain intentionally out of scope for this block."
    )
...
def _freeze_joint_regions_and_masks(
    benchmark: Stage3Benchmark, *, output_dir: Path
) -> Dict[str, Any]:
    ...
    candidate_mask_manifest = {
        "mask_specs": [
            mask.spec.to_dict() for _row_id, mask in sorted(candidate_masks.items())
        ],
        "mask_payload_digests": {
            row_id: mask.payload_digest
            for row_id, mask in sorted(candidate_masks.items())
        },
        "candidate_mask_digest": joint_candidate_mask_digest(
            tuple(mask.spec for mask in candidate_masks.values())
        ),
        "prototype_digest": _sha256(
            {key: value[2] for key, value in sorted(benchmark.prototypes.items())}
        ),
    ...
    pairwise_mask_manifest = {
        "pairwise_mask_specs": [
            mask.spec.to_dict() for _pair, mask in sorted(pairwise_masks.items())
        ],
        "pairwise_mask_payload_digests": {
            f"{row_a}|{row_b}": mask.payload_digest
            for (row_a, row_b), mask in sorted(pairwise_masks.items())
        },
        "pairwise_mask_digest": pairwise_mask_digest(
            tuple(mask.spec for mask in pairwise_masks.values())
        ),
    }
    same_action_pairs = sum(
        int(mask.spec.action_a == mask.spec.action_b)
    ...
    pairwise_mask_summary = {
        ...
        "nonzero_pairwise_mask_count": sum(
            int(mask.spec.pairwise_pixel_count > 0) for mask in pairwise_masks.values()
        ),
        "zero_mass_pair_count": sum(
            int(mask.spec.pairwise_pixel_count == 0) for mask in pairwise_masks.values()
        ),
        "same_action_pair_count": same_action_pairs,
        "different_action_pair_count": different_action_pairs,
        "maximum_pairwise_mass": max(
            mask.spec.pairwise_pixel_count for mask in pairwise_masks.values()
        ),
        "minimum_nonzero_pairwise_mass": min(
            mask.spec.pairwise_pixel_count
            for mask in pairwise_masks.values()
            if mask.spec.pairwise_pixel_count > 0
        ),
...
def _v3_calibration(
    *, architecture_id: str, benchmark: Stage3Benchmark, freeze: Mapping[str, Any]
) -> JointEvidenceCalibration:
    return JointEvidenceCalibration(
        ...
        region_spec_digest=freeze["region_manifest"]["region_spec_digest"],
        candidate_mask_digest=freeze["candidate_mask_manifest"][
...
def _direct_joint_ranked_candidates(
    *,
    ...
    calibration = _v3_calibration(
        architecture_id=architecture_id, benchmark=benchmark, freeze=freeze
    )
    ...
    return tuple(
        evaluate_joint_candidate_eligibility(
            candidate=item, ranked_candidates=ranked, calibration=calibration
        )
        for item in ranked
    )
...
def _freeze_benchmark_v3_into(output_dir: Path) -> Dict[str, Any]:
    ...
    generator_identity = _generator_identity_v3(benchmark)
    prototype_manifest = _prototype_manifest_v3(
        benchmark, generator_identity=generator_identity
    )
    development_manifest = _development_manifest_v3(
        benchmark, generator_identity=generator_identity
    )
...
def run_freeze_benchmark_v3(output_dir: Path) -> Dict[str, Any]:
    with tempfile.TemporaryDirectory(
        prefix="stage3-v3-freeze-", dir=str(REPO_ROOT)
    ) as tmp:
        temp_output = Path(tmp)
    ...
    return {
        ...
        "candidate_mask_digest": result["freeze"]["candidate_mask_manifest"][
            "candidate_mask_digest"
        ],
        "pairwise_mask_digest": result["freeze"]["pairwise_mask_manifest"][
            "pairwise_mask_digest"
        ],
    }
...
def run_audit_v3_self_retrieval(output_dir: Path) -> Dict[str, Any]:
    ...
    benchmark = _build_stage3_benchmark_v3(materialize_final=False)
    with tempfile.TemporaryDirectory(
        prefix="stage3-v3-audit-freeze-", dir=str(REPO_ROOT)
    ) as tmp:
    ...
    tie_safety = {
        "architectures": {},
        "incorrect_lexical_exact_accepts": 0,
        "equal_score_exact_accept_count": 0,
    }
    accounting = {
    ...
    for (row_a, row_b), mask in freeze["pairwise_masks"].items():
        ...
        if not np.array_equal(
            mask.pairwise_weights,
            freeze["pairwise_masks"][(row_a, row_b)].pairwise_weights,
        ):
            pairwise_symmetry["mask_symmetry_valid"] = False
    for architecture_id in ("A3", "B3", "C3", "D3"):
        ...
        for row_id, (
            _prototype_observation_id,
            action_id,
            _digest,
            observation,
        ) in sorted(benchmark.prototypes.items()):
            direct = _direct_joint_ranked_candidates(
                architecture_id=architecture_id,
                benchmark=benchmark,
                freeze=freeze,
                observation=observation,
            ...
            if [item.to_dict() for item in direct] != [
            ...
            candidate_set = build_joint_candidate_set(
                ranked_candidates=ranked, calibration=calibration
            )
            ...
            accounting["declared_informative_mass_total"] += float(
                expected.declared_informative_mass
            )
            accounting["stable_informative_mass_total"] += float(
                expected.stable_informative_mass
            )
            accounting["available_geometric_mass_total"] += float(
                expected.available_geometric_mass
            )
            accounting["available_candidate_fit_mass_total"] += float(
                expected.available_candidate_fit_mass
            )
            accounting["pairwise_discriminative_mass_total"] += float(
                expected.pairwise_discriminative_mass
            )
            ...
            rows.append(
                {
                    ...
                    "expected_candidate_strength": float(expected.candidate_strength),
                    "expected_candidate_scored_mass": float(
                        expected.actual_scored_mass
                    ),
                    ...
                    else float(runner_up.candidate_strength),
                    ...
                    "expected_row_candidate_set_eligibility": bool(
                        expected.eligible_for_candidate_set
                    ),
            ...
            if architecture_id == "C3":
                direct_by_row = {
                    item.competitor_row_id: item.margin
                    for item in expected.pairwise_evidence
                    if item.region_id == "target_band"
                }
                for candidate in ranked:
                    if candidate.row_id == row_id:
                        continue
                    reverse = next(
                        (
                            item.margin
                            for item in candidate.pairwise_evidence
                            if item.competitor_row_id == row_id
                            and item.region_id == "target_band"
                        ),
                        None,
                    )
                    ...
                    if (
                        ...
                        and abs(forward + reverse) > 1e-6
                    ):
                        pairwise_symmetry["pairwise_antisymmetry_valid"] = False
        status = (
            "eligible_self_retrieval"
            if top1 == len(benchmark.prototypes)
            else "ineligible_self_retrieval"
        )
        ...
        provider_equivalence["architectures"][architecture_id] = (
            arch_provider_equivalent
        )
    ...
    _write_json(
        output_dir / "canonical-self-retrieval-summary.json",
        {
    ...
    shared_valid = bool(
        provider_equivalence["all_match"]
        and pairwise_symmetry["mask_symmetry_valid"]
        and pairwise_symmetry["pairwise_antisymmetry_valid"]
        and incorrect_lexical_exact_accepts == 0
    )
    if shared_valid and any(
        item["self_retrieval_status"] == "eligible_self_retrieval"
        for item in by_arch.values()
    ):
        stop_outcome = (
            "Outcome B"
            if any(
    ...
    _write_json(
        output_dir / "architecture-self-retrieval-status.json",
        {
            "shared_instrument_valid": shared_valid,
            "architectures": by_arch,
...
def run_verify_v3_instrument(output_dir: Path) -> Dict[str, Any]:
    output_dir = OUTPUT_DIR_V3 if output_dir == OUTPUT_DIR else output_dir
    with tempfile.TemporaryDirectory(
        prefix="stage3-v3-verify-", dir=str(REPO_ROOT)
    ) as tmp:
        temp_output = Path(tmp)
        run_freeze_benchmark_v3(temp_output)
        run_audit_v3_self_retrieval(temp_output)
        artifacts = [
            name for name in V3_ARTIFACT_NAMES if name != "instrument-verification.json"
        ]
        comparison = _artifact_comparison(
            output_dir, temp_output, artifact_names=artifacts
        )
    forbidden = [
        name
        for name in (
...
def _stage2_diagnosis_file_digests(output_dir: Path) -> Dict[str, Any]:
    files = {
        ...
        "frame_candidates_csv": output_dir
        / "diagnostics"
        / "stage2-frame-candidates.csv",
        "correct_row_ranks_json": output_dir
        / "diagnostics"
        / "stage2-correct-row-ranks.json",
        "same_action_ranks_json": output_dir
        / "diagnostics"
        / "stage2-same-action-ranks.json",
        "conflicting_action_ranks_json": output_dir
        / "diagnostics"
        / "stage2-conflicting-action-ranks.json",
    }
    return {
        name: _sha256(path.read_text(encoding="utf-8")) for name, path in files.items()
    }
...
def _diagnose_stage2(output_dir: Path) -> Dict[str, Any]:
    ...
    region_visibility_by_family: dict[str, dict[str, list[float]]] = defaultdict(
        lambda: defaultdict(list)
    )
    region_overlap_by_family: dict[str, dict[str, list[float]]] = defaultdict(
        lambda: defaultdict(list)
    )
    region_distance_by_family: dict[str, dict[str, list[float]]] = defaultdict(
        lambda: defaultdict(list)
    )
    region_failure_by_family: dict[str, dict[str, int]] = defaultdict(
    ...
    candidate_rows = []
    for case in cases:
        for frame, expected_row, expected_action, _disposition in zip(
            case.source.frames(),
            case.expected_rows,
            ...
            if expected_row is not None:
                matching_rank = next(
                    (
                        ...
                        if candidate.row_id == expected_row
                    ),
                    None,
                )
                exact_rank_positions.append(
                    {
                        "family": case.family,
                ...
                for k in (1, 2, 3, 5):
                    topk_hits[f"top{k}"] += int(
                        matching_rank is not None and matching_rank <= k
                    )
                same_action_rank = next(
                    (
                        idx
                        for idx, candidate in enumerate(ranked, start=1)
                        if candidate.action_id == expected_action
                    ),
                    None,
                )
                same_action_rank_positions.append(
                    {
                        "family": case.family,
                        "frame_id": frame.frame_id,
                        "rank": same_action_rank,
                    }
                )
                for k in (1, 2, 3, 5):
                    topk_action_hits[f"top{k}"] += int(
                        same_action_rank is not None and same_action_rank <= k
                    )
                ...
                conflicting_action_rank_positions.append(
                    {
                        "family": case.family,
                        "frame_id": frame.frame_id,
                        "rank": conflicting_rank,
                    }
                )
            best = ranked[0]
            for region in best.region_evidence:
                region_visibility_by_family[case.family][region.region_id].append(
                    float(region.visible_fraction)
                )
                region_overlap_by_family[case.family][region.region_id].append(
                    float(region.overlap_fraction)
                )
                region_distance_by_family[case.family][region.region_id].append(
                    float(region.distance)
                )
                if (
                    float(region.visible_fraction) + 1e-12
            ...
            region_summary_rows.append(
                {
                    ...
                    "low_visibility_count": int(
                        region_failure_by_family[family].get(region_id, 0)
                    ),
                }
            )
    diagnostics = {
        ...
        "region_contract_digest": _sha256(
            [region.to_dict() for region in _stage2_regions()]
        ),
        "candidate_grid_digest": selection["candidate_grid_digest"],
        "family_rejections": {
            family: {
                reason: count
                for (fam, reason), count in sorted(family_rejections.items())
                if fam == family
            }
    ...
    _write_csv(
        output_dir / "diagnostics" / "stage2-region-summary.csv", region_summary_rows
    )
    _write_csv(
        output_dir / "diagnostics" / "stage2-frame-candidates.csv", candidate_rows
    )
    _write_json(
        output_dir / "diagnostics" / "stage2-correct-row-ranks.json",
        exact_rank_positions,
    )
    _write_json(
        output_dir / "diagnostics" / "stage2-same-action-ranks.json",
        same_action_rank_positions,
    )
    _write_json(
        output_dir / "diagnostics" / "stage2-conflicting-action-ranks.json",
        conflicting_action_rank_positions,
    )
...
def main() -> None:
    ...
    if args.verify_stage2_diagnosis:
        ...
        payload = run_freeze_benchmark_v2(
            OUTPUT_DIR_V2 if args.output_dir == OUTPUT_DIR else args.output_dir
        )
    elif args.verify_v2_benchmark:
        payload = run_verify_v2_benchmark(
            OUTPUT_DIR_V2 if args.output_dir == OUTPUT_DIR else args.output_dir
        )
    elif args.select_architecture_v2:
        payload = run_select_architecture_v2(
            OUTPUT_DIR_V2 if args.output_dir == OUTPUT_DIR else args.output_dir
        )
    elif args.calibrate_v2:
        payload = run_calibrate_v2(
            OUTPUT_DIR_V2 if args.output_dir == OUTPUT_DIR else args.output_dir
        )
    elif args.verify_pre_final_v2:
        payload = run_verify_pre_final_v2(
            OUTPUT_DIR_V2 if args.output_dir == OUTPUT_DIR else args.output_dir
        )
    elif args.audit_v2_representation:
        payload = _run_representation_audit_v2(
            OUTPUT_DIR_V2 if args.output_dir == OUTPUT_DIR else args.output_dir
        )
    elif args.evaluate_v2:
        payload = run_evaluate_v2(
            OUTPUT_DIR_V2 if args.output_dir == OUTPUT_DIR else args.output_dir
        )
    elif args.freeze_benchmark_v3:
        payload = run_freeze_benchmark_v3(
            OUTPUT_DIR_V3 if args.output_dir == OUTPUT_DIR else args.output_dir
        )
    elif args.audit_v3_self_retrieval:
        payload = run_audit_v3_self_retrieval(
            OUTPUT_DIR_V3 if args.output_dir == OUTPUT_DIR else args.output_dir
        )
    elif args.verify_v3_instrument:
        payload = run_verify_v3_instrument(
            OUTPUT_DIR_V3 if args.output_dir == OUTPUT_DIR else args.output_dir
        )
    else:
