...
def run(output_dir: Path, *, mode: str) -> dict[str, object]:
    ...
    _write(
        output_dir / "evaluation-summary.json",
        {
            ...
            "note": "Ordinary adjudication summary; not a privileged oracle.",
            "equivalent_baseline_comparison": "not_completed",
        },
    )
    ...
    _write(output_dir / "test-summary.json", _validation_summary(mode))
...
def _validation_summary(mode: str) -> dict[str, object]:
    return {
        ...
        "status": "passed",
        "focused_tests": {
            "command": "python -m pytest examples/visual_transition_benchmark/tests/test_adjudication.py -q",
            "passed": 16,
            "failed": 0,
        },
        "fast_suite": {
