...
from zeromodel.video.arcade_policy.rendering import COOLDOWN_BLOCKED_VALUE, COOLDOWN_READY_VALUE
...
TANK_BAND = RegionGeometry(
    region_id="tank_band", canvas_shape=CANVAS_SHAPE,
    y0=11, y1=14, x0=0, x1=ds.WIDTH_PX, cell_height=3, cell_width=ds.CELL_PIXELS,
)
ALIEN_BAND = RegionGeometry(
    region_id="alien_band", canvas_shape=CANVAS_SHAPE,
    y0=2, y1=5, x0=0, x1=ds.WIDTH_PX, cell_height=3, cell_width=ds.CELL_PIXELS,
)
COOLDOWN_REGION = RegionGeometry(
    region_id="cooldown_region", canvas_shape=CANVAS_SHAPE,
    y0=7, y1=9, x0=ds.WIDTH_PX - ds.CELL_PIXELS, x1=ds.WIDTH_PX, cell_height=1, cell_width=ds.CELL_PIXELS,
)
...
def _tank_position_samples(count: int, seed: int) -> Tuple[DevelopmentSample, ...]:
    ...
    for i, category in enumerate(ds.ORDINARY_CATEGORIES):
        for j in range(count):
            rec = ds.build_transition(episode_id=f"tank-{category}", step_number=j, seed=seed + i * 1000 + j, category=category)
            samples.append(DevelopmentSample(
                ...
                is_unrelated=(rec.state_before["tank_x"] == rec.state_after["tank_x"]),
            ))
...
def _cooldown_samples(count: int, seed: int) -> Tuple[DevelopmentSample, ...]:
    ...
    for i, category in enumerate(ds.ORDINARY_CATEGORIES):
        for j in range(count):
            rec = ds.build_transition(episode_id=f"cooldown-{category}", step_number=j, seed=seed + i * 1000 + j, category=category)
...
def _presence_samples(band: str, region: RegionGeometry, count: int, seed: int) -> Tuple[DevelopmentSample, ...]:
    ...
    for i, category in enumerate(ds.ALL_CATEGORIES):
        for j in range(count):
            rec = ds.build_transition(episode_id=f"presence-{band}-{category}", step_number=j, seed=seed + i * 1000 + j, category=category)
            before_patch = rec.frame_before[region.y0:region.y1, region.x0:region.x1]
...
def _identity_samples(count: int, seed: int) -> Tuple[DevelopmentSample, ...]:
    ...
    for i, category in enumerate(ds.ORDINARY_CATEGORIES):
        for j in range(count):
            rec = ds.build_transition(episode_id=f"identity-{category}", step_number=j, seed=seed + i * 1000 + j, category=category)
            samples.append(DevelopmentSample(
                sample_id=rec.transition_id,
                frame_before=rec.frame_before,
                frame_after=rec.frame_after,
...
def build_cases() -> Tuple[CompilerCase, ...]:
    return (
        CompilerCase(
            name="tank_presence",
            requirement=VisualEvidenceRequirement(
                domain_name="arcade", component_type="tank", property_name="presence",
                evidence_kind="presence", candidate_region_id="tank_band", comparison="changed",
            ),
            region=TANK_BAND, canonical_levels=(), min_decoding_accuracy=0.95,
            build_samples=lambda count, seed: _presence_samples("tank", TANK_BAND, count, seed),
        ),
        CompilerCase(
            name="tank_position",
            requirement=VisualEvidenceRequirement(
                domain_name="arcade", component_type="tank", property_name="position",
                evidence_kind="spatial_position", candidate_region_id="tank_band", comparison="equal",
            ),
            region=TANK_BAND, canonical_levels=(), min_decoding_accuracy=0.95,
        ...
        CompilerCase(
            name="tank_direction",
            requirement=VisualEvidenceRequirement(
                domain_name="arcade", component_type="tank", property_name="direction",
                evidence_kind="signed_delta", candidate_region_id="tank_band", comparison="signed_delta",
            ),
            region=TANK_BAND, canonical_levels=(), min_decoding_accuracy=0.95,
            build_samples=_tank_position_samples,
        ),
        CompilerCase(
            ...
            requirement=VisualEvidenceRequirement(
                domain_name="arcade", component_type="tank", property_name="movement_magnitude",
                evidence_kind="exact_magnitude", candidate_region_id="tank_band", comparison="exact_delta",
            ),
            region=TANK_BAND, canonical_levels=(), min_decoding_accuracy=0.95,
            build_samples=_tank_position_samples,
        ),
        CompilerCase(
            name="cooldown_value",
            requirement=VisualEvidenceRequirement(
                domain_name="arcade", component_type="cooldown", property_name="cooldown_value",
                evidence_kind="numeric_value", candidate_region_id="cooldown_region",
                expected_value_domain=(READY, BLOCKED), required_precision=15 / 255, comparison="equal",
            ),
            region=COOLDOWN_REGION, canonical_levels=(READY, BLOCKED), min_decoding_accuracy=0.95,
            build_samples=_cooldown_samples,
        ),
        CompilerCase(
            name="alien_target_identity",
            requirement=VisualEvidenceRequirement(
                domain_name="arcade", component_type="alien", property_name="target_identity",
                evidence_kind="visible_identity", candidate_region_id="alien_band",
                comparison="identity_equal", permits_identity_marker=True,
            ),
            region=ALIEN_BAND, canonical_levels=(), min_decoding_accuracy=0.95,
