...
INTERIOR_REGION = RegionGeometry(
    region_id="interior_grid",
    canvas_shape=CANVAS_SHAPE,
    y0=0,
    y1=wm.GRID_SIZE * wr.CELL_PIXELS,
    x0=0,
    x1=wr.CANVAS_WIDTH,
    cell_height=wr.CELL_PIXELS,
    cell_width=wr.CELL_PIXELS,
)
BATTERY_SEGMENT_WIDTH = wr.CANVAS_WIDTH // wm.MAX_BATTERY
BATTERY_REGION = RegionGeometry(
    region_id="battery_strip",
    canvas_shape=CANVAS_SHAPE,
    y0=wm.GRID_SIZE * wr.CELL_PIXELS,
    y1=wr.CANVAS_HEIGHT,
    x0=0,
    x1=wr.CANVAS_WIDTH,
    cell_height=wr.BATTERY_STRIP_HEIGHT,
    cell_width=BATTERY_SEGMENT_WIDTH,
)
...
DOOR_REGION = RegionGeometry(
    ...
    canvas_shape=CANVAS_SHAPE,
    y0=_door_y0,
    y1=_door_y0 + wr.CELL_PIXELS,
    x0=_door_x0,
    x1=_door_x0 + wr.CELL_PIXELS,
    cell_height=wr.CELL_PIXELS,
    cell_width=wr.CELL_PIXELS,
)
...
CRATE_REGION = RegionGeometry(
    region_id="crate_probe_cell",
    canvas_shape=CANVAS_SHAPE,
    y0=_crate_y0,
    y1=_crate_y0 + wr.CELL_PIXELS,
    x0=_crate_x0,
    x1=_crate_x0 + wr.CELL_PIXELS,
    cell_height=wr.CELL_PIXELS,
    cell_width=wr.CELL_PIXELS,
    sub_patch_size=2,
)
...
BATTERY_LEVELS: Tuple[float, ...] = tuple(
    (k / wm.MAX_BATTERY) * (wr.BATTERY_ON_VALUE / 255)
    for k in range(wm.MAX_BATTERY + 1)
)
...
def _door_samples(count: int, seed: int) -> Tuple[DevelopmentSample, ...]:
    ...
    for i, category in enumerate(wf.ALL_CATEGORIES):
        for j in range(count):
            rec = wf.build_transition(
                episode_id=f"door-{category}",
                step_number=j,
                seed=seed + i * 1000 + j,
                category=category,
            )
            true_before = (
                DOOR_OPEN_LEVEL if rec.state_before["door_open"] else DOOR_CLOSED_LEVEL
            )
            true_after = (
                DOOR_OPEN_LEVEL
                if rec.rendered_state["door_open"]
                else DOOR_CLOSED_LEVEL
            )
            samples.append(
                DevelopmentSample(
                    sample_id=rec.transition_id,
...
def _crate_identity_samples(count: int, seed: int) -> Tuple[DevelopmentSample, ...]:
    ...
    for identity in range(wm.MAX_CRATES):
        for variant in range(count):
            fillers = [c for c in filler_cells if c != (_crate_row, _crate_col)][
                :identity
            ]
            crates = tuple(fillers) + ((_crate_row, _crate_col),)
            robot_cell = (1, 1) if (1, 1) not in crates else (3, 3)
            state = wm.WarehouseState(
                robot=robot_cell,
                crates=crates,
                door_open=(variant % 2 == 0),
                battery=variant % (wm.MAX_BATTERY + 1),
            )
    ...
    return (
        CompilerCase(
            ...
            requirement=VisualEvidenceRequirement(
                ...
                candidate_region_id="interior_grid",
                comparison="equal",
            ),
        ...
        CompilerCase(
            ...
            requirement=VisualEvidenceRequirement(
                ...
                candidate_region_id="battery_strip",
                expected_value_domain=BATTERY_LEVELS,
                required_precision=20 / 255,
                comparison="equal",
            ),
            region=BATTERY_REGION,
            canonical_levels=BATTERY_LEVELS,
            sub_patch_offsets=(),
            min_decoding_accuracy=0.95,
        ...
        CompilerCase(
            ...
            requirement=VisualEvidenceRequirement(
                ...
                component_type="door",
                property_name="state",
                evidence_kind="categorical_state",
                candidate_region_id="door_cell",
                comparison="equal",
            ),
            region=DOOR_REGION,
            canonical_levels=(DOOR_CLOSED_LEVEL, DOOR_OPEN_LEVEL),
            sub_patch_offsets=(),
            min_decoding_accuracy=0.95,
