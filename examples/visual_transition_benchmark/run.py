...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("artifacts/visual_transition_benchmark")
    )
    parser.add_argument(
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        dev_split = ds.generate_split(
            prefix="dev", episode_count=args.dev_episodes, seed_offset=0
        )
        ...
        metrics_report = rp.build_metrics_report(
            eval_split.records, eval_zm, eval_pd, eval_priv
        )

        output_dir = args.output_dir
        output_dir.mkdir(parents=True, exist_ok=True)
        rp.write_transition_level_results(
            output_dir / "transition-level-results.jsonl",
            eval_split.records,
            eval_zm,
            eval_pd,
            eval_priv,
        ...
        if not args.skip_render:
            ...
            render_episode_ids = set(
                eval_split.episode_ids[: args.render_ordinary_episodes]
            )
            ordinary_sample = [
                (r, z, p)
                for r, z, p in zip(eval_split.records, eval_zm, eval_pd)
            ...
            priv_by_id = {
                r.transition_id: p for r, p in zip(eval_split.records, eval_priv)
            }
            for record, zm_out, pd_out in faulty + ordinary_sample:
                ...
                zm_f1 = mx.per_transition_component_f1(
                    zm_out.predicted_components, truth
                )
                pd_f1 = mx.per_transition_component_f1(
                    pd_out.predicted_components, truth
                )
                ...
                artifact_rows.append(
                    {
                        ...
                        "false_positive": bool(
                            not record.is_faulty
                            and (
                                zm_out.missing_components
                                or zm_out.unexpected_components
                            )
                        ),
                        "false_negative": bool(
                            record.is_faulty
            ...
            build_html_index(
                artifact_rows, output_path=output_dir / "visual-index.html"
            )
