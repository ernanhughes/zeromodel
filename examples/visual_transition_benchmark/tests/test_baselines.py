...
def test_pixel_diff_cannot_represent_an_absent_expected_transition():
    ...
    record = ds.build_transition(
        episode_id="e", step_number=0, seed=1, category="expected_target_remains_unchanged"
    )
...
def test_privileged_baseline_matches_declared_component_changes():
    for category in ds.ALL_CATEGORIES:
        record = ds.build_transition(episode_id="e", step_number=0, seed=2, category=category)
        output = bl.privileged_baseline(record)
        assert set(output.predicted_components) == set(record.observed_changed_components)
        assert output.missing_components == tuple(
            sorted(set(record.expected_changed_components) - set(record.observed_changed_components))
        )
        assert output.unexpected_components == tuple(
            sorted(set(record.observed_changed_components) - set(record.expected_changed_components))
        )
