...
def test_compiler_run_end_to_end_small(tmp_path: Path):
    ...
    payload = json.loads(
        (tmp_path / "compiler-results.json").read_text(encoding="utf-8")
    )
    ...
    assert (
        by_case[("arcade", "alien_target_identity")]["status"]
        == "insufficient_observability"
    )
    ...
    for (domain, case_name), r in by_case.items():
        ...
        if (domain, case_name) == ("arcade", "alien_target_identity"):
            ...
            assert manual["candidate"] is not None, (
                f"{domain}/{case_name} missing a manual reference"
            )
