...
CANVAS = (8, 8)
REGION = RegionGeometry(region_id="r", canvas_shape=CANVAS, y0=0, y1=2, x0=0, x1=4, cell_height=1, cell_width=2)
...
def _presence_requirement():
    return VisualEvidenceRequirement(
        domain_name="synthetic", component_type="probe", property_name="presence",
        evidence_kind="presence", candidate_region_id="r", comparison="changed",
    )
...
def test_compiles_when_a_candidate_clearly_recovers_the_property():
    ...
    samples = [
        DevelopmentSample(sample_id="s0", frame_before=_frame(0), frame_after=_frame(200), true_before=False, true_after=True),
        DevelopmentSample(sample_id="s1", frame_before=_frame(50), frame_after=_frame(50), true_before=False, true_after=False),
        DevelopmentSample(sample_id="s2", frame_before=_frame(0), frame_after=_frame(180), true_before=False, true_after=True),
        DevelopmentSample(sample_id="s3", frame_before=_frame(30), frame_after=_frame(30), true_before=False, true_after=False),
    ]
    compiled = compile_requirement(req, REGION, candidates, samples, min_decoding_accuracy=0.95)
...
def test_selection_is_deterministic_across_repeated_runs():
    ...
    a = compile_requirement(req, REGION, candidates, samples, min_decoding_accuracy=0.95)
    b = compile_requirement(req, REGION, candidates, samples, min_decoding_accuracy=0.95)
...
def test_insufficient_observability_when_every_candidate_is_degenerate():
    ...
    req = VisualEvidenceRequirement(
        domain_name="synthetic", component_type="probe", property_name="hidden_value",
        evidence_kind="numeric_value", candidate_region_id="r",
        expected_value_domain=(0.1, 0.9), required_precision=0.05, comparison="equal",
    )
    candidates = generate_candidates(req, REGION)
    samples = [
        DevelopmentSample(sample_id=f"s{i}", frame_before=_frame(0), frame_after=_frame(0), true_before=0.1, true_after=(0.1 if i % 2 == 0 else 0.9))
        for i in range(6)
    ]
    compiled = compile_requirement(req, REGION, candidates, samples, canonical_levels=(0.1, 0.9), min_decoding_accuracy=0.95)
...
def test_insufficient_representation_when_signal_exists_but_no_candidate_passes():
    ...
    req = VisualEvidenceRequirement(
        domain_name="synthetic", component_type="probe", property_name="uncorrelated_value",
        evidence_kind="numeric_value", candidate_region_id="r",
        expected_value_domain=(0.1, 0.9), required_precision=0.02, comparison="equal",
    )
    candidates = generate_candidates(req, REGION)
    ...
    samples = [
        DevelopmentSample(
            sample_id=f"s{i}", frame_before=_frame(0), frame_after=_frame(fills[i]),
            true_before=0.1, true_after=(0.1 if i % 2 == 0 else 0.9),
        )
...
def test_compiled_and_insufficient_statuses_are_never_conflated():
    ...
    compiled = compile_requirement(req, REGION, candidates, samples, min_decoding_accuracy=0.95)
    assert compiled.status in ("compiled", "insufficient_representation", "insufficient_observability")
