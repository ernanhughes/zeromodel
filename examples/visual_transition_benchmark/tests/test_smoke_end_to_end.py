...
def test_small_deterministic_dataset_end_to_end(tmp_path: Path):
    ...
    metrics_report = rp.build_metrics_report(eva.records, zm_outputs, pd_outputs, priv_outputs)
    assert metrics_report["all"]["n"] == len(eva.records)
    assert metrics_report["ordinary"]["n"] + metrics_report["faulty"]["n"] == len(eva.records)
    ...
    rp.write_transition_level_results(jsonl_path, eva.records, zm_outputs, pd_outputs, priv_outputs)
    ...
    png_path = render_transition_panel(
        faulty_record, faulty_priv, faulty_zm, output_path=tmp_path / "artifacts" / "one.png"
    )
