...
def test_decoded_values_match_ground_truth_on_ordinary_transitions():
    for category in ds.ORDINARY_CATEGORIES:
        for seed in range(10):
            record = ds.build_transition(episode_id="e", step_number=0, seed=seed, category=category)
            values = _decode(record)
            assert values.tank.before_x == record.state_before["tank_x"]
            assert values.tank.after_x == record.state_after["tank_x"]
            expected_cooldown = "blocked" if record.state_after["cooldown"] == 1 else "ready"
...
def test_contracts_catch_cooldown_out_of_domain_values():
    for category in ("cooldown_activates_with_wrong_value", "cooldown_decreases_to_wrong_value"):
        record = ds.build_value_transition(episode_id="e", step_number=0, seed=2, category=category)
        values = _decode(record)
        verdict = vc.evaluate_contracts(record.action, values)
...
def test_contracts_remain_honestly_blind_to_target_identity_faults():
    ...
    for category in ("wrong_alien_disappears", "two_aliens_disappear_instead_of_one"):
        record = ds.build_value_transition(episode_id="e", step_number=0, seed=3, category=category)
        values = _decode(record)
        verdict = vc.evaluate_contracts(record.action, values)
    ...
    for category in ds.ORDINARY_CATEGORIES:
        ...
        for seed in range(30):
            ...
            if (
                ...
                false_alarms += 1
    assert false_alarms == 0, f"{false_alarms}/{total} legitimate transitions were flagged"
