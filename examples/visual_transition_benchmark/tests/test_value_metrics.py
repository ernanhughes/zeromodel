...
class _FakeRecord:
    def __init__(
        self, tank_before, tank_after, cooldown_after, target_after, is_faulty=True
    ):
        self.state_before = {"tank_x": tank_before}
        self.state_after = {
            "tank_x": tank_after,
            "cooldown": cooldown_after,
            "target_x": target_after,
        }
...
def _values(tank_delta, cooldown_level, target_after, tank_before=0):
    return DecodedValues(
        tank=TankValues(
            before_x=tank_before, after_x=tank_before + tank_delta, delta_x=tank_delta
        ),
        alien=AlienValues(
            before_alive=True,
            after_alive=target_after is not None,
            before_x=0,
            after_x=target_after,
        ),
        cooldown=CooldownValues(
            before_intensity=0.0,
            after_intensity=0.0,
            before_level="ready",
            after_level=cooldown_level,
        ),
    )
...
def test_accuracy_perfect_match():
    ...
    values = _values(
        tank_delta=-1, cooldown_level="blocked", target_after=5, tank_before=2
    )
...
def test_accuracy_wrong_direction_fails_both_direction_and_delta():
    record = _FakeRecord(
        tank_before=2, tank_after=1, cooldown_after=0, target_after=None
    )
    values = _values(
        tank_delta=1, cooldown_level="ready", target_after=None, tank_before=2
    )  # moved +1, true was -1
    summary = vm.value_accuracy_summary([record], [values])
...
def test_value_fault_present_flags_any_wrong_dimension():
    ...
    correct = _values(
        tank_delta=0, cooldown_level="blocked", target_after=5, tank_before=2
    )
    wrong_cooldown = _values(
        tank_delta=0, cooldown_level="out_of_domain", target_after=5, tank_before=2
    )
...
def test_fault_localization_detects_and_does_not_false_alarm():
    faulty_record = _FakeRecord(
        tank_before=0, tank_after=0, cooldown_after=1, target_after=5
    )
    faulty_values = _values(
        tank_delta=0, cooldown_level="out_of_domain", target_after=5, tank_before=0
    )
    clean_record = _FakeRecord(
        tank_before=0,
        tank_after=1,
        cooldown_after=0,
        target_after=None,
    ...
    clean_values = _values(
        tank_delta=1, cooldown_level="ready", target_after=None, tank_before=0
    )
...
def test_label_correct_but_value_wrong_counts_the_hidden_failure_mode():
    record = _FakeRecord(
        tank_before=2, tank_after=1, cooldown_after=0, target_after=None, is_faulty=True
    )
    ...
    wrong_values = _values(
        tank_delta=1, cooldown_level="ready", target_after=None, tank_before=2
    )

    summary = vm.label_correct_but_value_wrong(
        [record], [component_output], [wrong_values]
    )
