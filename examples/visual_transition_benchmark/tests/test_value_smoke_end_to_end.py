...
def test_small_deterministic_value_dataset_end_to_end(tmp_path: Path):
    ...
    value_new = ds.generate_value_split(prefix="v-smoke-new", episode_count=2, seed_offset=500)
    ...
    direction_record = next(r for r in records if r.category == "tank_moves_wrong_direction")
    ...
    rp.write_json(results_path, {"accuracy": accuracy.__dict__, "localization": localization.__dict__})
