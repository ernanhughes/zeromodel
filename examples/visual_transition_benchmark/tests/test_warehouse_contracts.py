...
def test_door_bar_is_not_diluted_by_the_cells_empty_majority():
    ...
    record = wf.build_transition(episode_id="e", step_number=0, seed=1, category="door_opens")
...
def test_no_false_alarms_on_ordinary_categories_except_documented_boundary_cases():
    documented_false_alarms = {"robot_blocked_by_wall", "push_attempt_with_no_crate_is_noop"}
    for category in wf.ORDINARY_CATEGORIES:
        for seed in range(15):
            record = wf.build_transition(episode_id="e", step_number=0, seed=seed, category=category)
            component, value = _analyze(record)
            flagged = bool(component.missing_components or component.unexpected_components or value.value_flags)
            if category in documented_false_alarms:
                assert flagged
            else:
                assert not flagged, f"{category} seed={seed} unexpectedly flagged: {component}, {value}"


def test_presence_faults_are_caught_at_component_level():
    for category in wf.PRESENCE_FAULT_CATEGORIES:
        record = wf.build_transition(episode_id="e", step_number=0, seed=2, category=category)
...
def test_value_faults_are_caught_only_at_value_level():
    for category in wf.VALUE_FAULT_CATEGORIES:
        record = wf.build_transition(episode_id="e", step_number=0, seed=2, category=category)
        component, value = _analyze(record)
        assert not component.missing_components and not component.unexpected_components, (
...
def test_relation_faults_are_caught_via_the_adjacency_relation():
    ...
    component_clean_categories = {"push_advances_robot_without_crate", "two_crates_move_during_single_push"}
    for category in wf.RELATION_FAULT_CATEGORIES:
        ...
        if category == "push_advances_robot_without_crate":
            ...
            assert any(flag.startswith("relation:") for flag in value.value_flags), category
...
def test_identity_faults_remain_an_honest_blind_spot_for_value_flags():
    ...
    for category in wf.IDENTITY_FAULT_CATEGORIES:
        ...
        assert not any(f.startswith("relation:") for f in value.value_flags) or category in (
...
def test_decode_matches_ground_truth_on_ordinary_transitions():
    for category in wf.ORDINARY_CATEGORIES:
        record = wf.build_transition(episode_id="e", step_number=0, seed=4, category=category)
        grids = wc.build_transition_evidence(record.frame_before, record.frame_after)
