...
def test_value_and_identity_faults_are_label_correct():
    ...
    for category in label_correct_families:
        record = wf.build_transition(episode_id="e", step_number=0, seed=3, category=category)
...
def test_push_based_relation_faults_are_label_correct():
    ...
    for category in ("push_advances_robot_without_crate", "two_crates_move_during_single_push"):
        record = wf.build_transition(episode_id="e", step_number=0, seed=3, category=category)
        assert record.is_faulty
        assert set(record.observed_changed_components) == set(record.expected_changed_components)
...
def test_all_categories_constructible_across_many_seeds():
    for seed in range(30):
        for category in wf.ALL_CATEGORIES:
            record = wf.build_transition(episode_id=f"seed-{seed}", step_number=0, seed=seed, category=category)
...
def test_battery_and_robot_bounds_never_violated():
    for seed in range(30):
        for category in wf.ALL_CATEGORIES:
            record = wf.build_transition(episode_id=f"bounds-{seed}", step_number=0, seed=seed, category=category)
            assert 0 <= record.state_after["battery"] <= wm.MAX_BATTERY
            row, col = record.state_after["robot"]
            assert not wm.is_wall((row, col)) or record.category == "robot_blocked_by_wall"
