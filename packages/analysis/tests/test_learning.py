...
def test_learning_vpm_requires_train_heldout_and_regression_evidence() -> None:
    assessment = build_learning_vpm(
        [
            LearningObservation(
                unit_id="claim-a", before=0.40, after=0.70, split="train"
            ),
            LearningObservation(
                unit_id="claim-b", before=0.50, after=0.62, split="heldout"
            ),
            LearningObservation(
                unit_id="claim-c", before=0.80, after=0.80, split="regression"
            ),
        ]
    )
...
def test_regression_prevents_learning_claim() -> None:
    assessment = build_learning_vpm(
        [
            LearningObservation(
                unit_id="claim-a", before=0.40, after=0.75, split="train"
            ),
            LearningObservation(
                unit_id="claim-b", before=0.50, after=0.64, split="heldout"
            ),
            LearningObservation(
                unit_id="claim-c", before=0.90, after=0.70, split="regression"
            ),
        ]
    )
...
def test_learning_vpm_cell_maps_to_learning_observation() -> None:
    assessment = build_learning_vpm(
        [
            LearningObservation(
                unit_id="train-unit", before=0.3, after=0.6, split="train"
            ),
            LearningObservation(
                unit_id="heldout-unit", before=0.4, after=0.55, split="heldout"
            ),
            LearningObservation(
                unit_id="regression-unit", before=0.9, after=0.9, split="regression"
            ),
        ]
    )
    ...
    assert first_cell.row_id in {
        "train:train-unit",
        "heldout:heldout-unit",
...
def test_learning_vpm_rejects_duplicate_split_unit_rows() -> None:
    with pytest.raises(VPMValidationError, match="unique split:unit_id"):
        build_learning_vpm(
            [
                LearningObservation(
                    unit_id="same", before=0.1, after=0.2, split="train"
                ),
                LearningObservation(
                    unit_id="same", before=0.2, after=0.3, split="train"
                ),
            ]
        )
