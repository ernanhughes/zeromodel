...
GOLDEN_SAMPLE_ARTIFACT_ID = (
    "32f801671139b73e349c756570c27c06d39c422a4d9a277782e1c997a473083b"
)
...
def test_build_vpm_is_deterministic_and_preserves_golden_identity() -> None:
    ...
    assert first.artifact_id == GOLDEN_SAMPLE_ARTIFACT_ID
    assert first.normalized_values.flags.writeable is False


def test_cell_region_and_order_mapping_are_source_stable() -> None:
...
def test_score_table_and_recipe_validation_errors() -> None:
    with pytest.raises(VPMValidationError, match="Duplicate row"):
        ScoreTable(
            values=[[0.1], [0.2]], row_ids=["same", "same"], metric_ids=["quality"]
        )
...
def test_artifact_round_trip_and_tamper_rejection() -> None:
    artifact = build_vpm(sample_table(), quality_recipe())
    loaded = VPMArtifact.from_dict(
        json.loads(json.dumps(artifact.to_dict(), sort_keys=True))
    )
