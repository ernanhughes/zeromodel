...
def test_identical_input_and_spec_yield_identical_hierarchy_identity(compiler_spec):
    artifacts = make_source_artifacts(5)
    store_a = InMemoryArtifactStore()
    store_b = InMemoryArtifactStore()
    manifest_a = compile_hierarchy(
        spec=compiler_spec, source_artifacts=artifacts, store=store_a
    )
    manifest_b = compile_hierarchy(
        spec=compiler_spec, source_artifacts=artifacts, store=store_b
    )
    assert manifest_a.hierarchy_id == manifest_b.hierarchy_id
    assert manifest_a.root_ref == manifest_b.root_ref


def test_changed_child_order_changes_hierarchy_identity(compiler_spec, artifact_store):
    artifacts = make_source_artifacts(5)
    reordered = (artifacts[1], artifacts[0], *artifacts[2:])
    manifest_original = compile_hierarchy(
        spec=compiler_spec, source_artifacts=artifacts, store=InMemoryArtifactStore()
    )
    manifest_reordered = compile_hierarchy(
        spec=compiler_spec, source_artifacts=reordered, store=InMemoryArtifactStore()
    )
    assert manifest_original.hierarchy_id != manifest_reordered.hierarchy_id


def test_changed_spec_rule_changes_hierarchy_identity(compiler_spec):
    artifacts = make_source_artifacts(5)
    manifest_original = compile_hierarchy(
        spec=compiler_spec, source_artifacts=artifacts, store=InMemoryArtifactStore()
    )
    ...
    manifest_changed = compile_hierarchy(
        spec=changed_spec, source_artifacts=artifacts, store=InMemoryArtifactStore()
    )
    assert manifest_original.hierarchy_id != manifest_changed.hierarchy_id


def test_incompatible_corpus_artifact_fails_compilation(compiler_spec, artifact_store):
...
def test_max_depth_enforced_at_compile_time(compiler_spec, artifact_store):
    ...
    artifacts = make_source_artifacts(9)
    with pytest.raises(HierarchyCompilationError):
        compile_hierarchy(
...
def test_root_and_every_reference_resolves_after_compilation(
    ...
    artifacts = make_source_artifacts(7)
...
def test_missing_leaf_reference_fails_closure(compiler_spec, artifact_store):
...
def test_self_referencing_tile_fails_closure(
    compiler_spec, artifact_store, monkeypatch
):
    ...
    fake_tile = types.SimpleNamespace(
        ...
        coverage=types.SimpleNamespace(child_count=1),
    )
...
def test_cyclic_hierarchy_fails_closure(compiler_spec, artifact_store, monkeypatch):
...
def test_navigation_owns_no_independent_persistence(compiler_spec, artifact_store):
