...
CERTIFICATION_AUDIT_FINDING_VERSION: Final = (
    "perception-certification-integrity-finding/1"
)
...
class CertificationIntegrityFindingDTO:
    ...
    def __post_init__(self) -> None:
        ...
        if self.related_ids != tuple(sorted(set(self.related_ids))):
            raise PerceptionCertificationAuditError(
                "related identities must be sorted and unique"
            )
...
class CertificationIntegrityAuditReportDTO:
    ...
    def __post_init__(self) -> None:
        if not self.report_id or not self.governance_audit_report_id:
            raise PerceptionCertificationAuditError(
                "report identities must be non-empty"
            )
        ...
        if self.findings != expected:
            raise PerceptionCertificationAuditError(
                "findings must be canonically sorted"
            )
        if (
            self.semantics != CERTIFICATION_AUDIT_SEMANTICS
            or self.version != CERTIFICATION_AUDIT_VERSION
        ):
...
def audit_certification_integrity(
    ...
    recommendations = {
        item.recommendation_id: item for item in governance_store.list_recommendations()
    }
    dispositions = {
        item.disposition_id: item for item in governance_store.list_dispositions()
    }
    receipts = {
        item.receipt_id: item for item in governance_store.list_execution_receipts()
    }
    ...
    if base.status != "valid":
        ...
        findings.append(
            _finding(
                severity,
                f"underlying_governance_{base.status}",
                "governance_audit",
                base.report_id,
                "three-store governance integrity is not valid",
            )
        )
            ...
            if owned is None:
                findings.append(
                    _finding(
                        ...
                        "certification references a missing artifact",
                        related,
                    )
                )
        if attempt is not None and (
            attempt.disposition_id != item.disposition_id
            or attempt.recommendation_id != item.recommendation_id
        ):
            findings.append(
                _finding(
                    "error",
                    "certification_attempt_ownership_mismatch",
                    "certification",
                    item.certification_id,
                    "attempt ownership differs from certification",
                    attempt.attempt_id,
                )
            )
        if (
            disposition is not None
            and disposition.recommendation_id != item.recommendation_id
        ):
            findings.append(
                _finding(
                    "error",
                    "certification_disposition_ownership_mismatch",
                    "certification",
                    item.certification_id,
                    "disposition ownership differs from certification",
                    disposition.disposition_id,
                )
            )
        if receipt is not None:
            if (
                receipt.disposition_id != item.disposition_id
                or receipt.recommendation_id != item.recommendation_id
            ):
                findings.append(
                    _finding(
                        "error",
                        "certification_receipt_ownership_mismatch",
                        "certification",
                        item.certification_id,
                        "receipt ownership differs from certification",
                        receipt.receipt_id,
                    )
                )
            if receipt.pointer_revision != item.resulting_pointer_revision:
                findings.append(
                    _finding(
                        "error",
                        "certification_receipt_revision_mismatch",
                        "certification",
                        item.certification_id,
                        "receipt revision differs from certification",
                        receipt.receipt_id,
                    )
                )
    ...
    for attempt in attempts.values():
        ...
        if terminal is None or terminal.event_kind not in {
            "completed",
            "reconciled",
            "idempotent",
        }:
        ...
        if attempt.attempt_id not in certified_attempts:
            findings.append(
                _finding(
                    "warning",
                    "successful_attempt_uncertified",
                    "attempt",
                    attempt.attempt_id,
                    "successful execution has no durable certification",
    ...
    status = (
        "invalid"
        if any(item.severity == "error" for item in canonical)
        else "attention_required"
        if any(item.severity == "warning" for item in canonical)
        else "valid"
    )
    payload: Mapping[str, object] = {
        "status": status,
