...
from zeromodel.video.video import VideoClipManifest, VideoFrame, VideoFrameSource
...
def _json_bytes(value: Any) -> bytes:
    try:
        ...
        raise VPMValidationError(
            "video policy values must be JSON-serializable"
        ) from exc
...
class TemporalEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "unmatched_duration_frames", int(self.unmatched_duration_frames)
        )
        ...
        object.__setattr__(
            self, "evidence_window", tuple(str(item) for item in self.evidence_window)
        )
    ...
    def __post_init__(self) -> None:
        ...
        if self.accepted:
            if self.policy is None or not self.address.accepted:
                raise VPMValidationError(
                    "accepted video decision needs address and policy evidence"
                )
            if self.policy.row_id != self.address.matched_row_id:
                raise VPMValidationError("accepted address and policy rows must match")
            if reasons:
                raise VPMValidationError(
                    "accepted video decision cannot have rejection reasons"
                )
        elif self.policy is not None or not reasons:
            raise VPMValidationError(
                "rejected video decision needs reasons and no executed policy"
            )
...
class VideoPolicyReader:
    def __init__(
        ...
        *,
        ...
        if set(transition_spec.row_ids) != set(
            str(item) for item in policy_lookup.artifact.source.row_ids
        ):
            raise VPMValidationError(
                "transition spec must cover the exact policy row set"
            )
        if int(evidence_window_size) < 1:
            raise VPMValidationError("evidence_window_size must be positive")
        if (
            maximum_identical_frame_run is not None
            and int(maximum_identical_frame_run) < 1
        ):
        ...
        self.maximum_identical_frame_run = (
            None
            if maximum_identical_frame_run is None
    ...
    @staticmethod
    def _validate_frame(
        manifest: VideoClipManifest, frame: VideoFrame, index: int
    ) -> None:
        ...
        expected: tuple[int, ...] = (manifest.height, manifest.width)
    ...
    def read(self, source: VideoFrameSource) -> VideoPolicyTrace:
        ...
        decisions: list[VideoPolicyDecision] = []
        ...
        last_pixel_digest: str | None = None
        window: list[str] = []
        ...
        for index, frame in enumerate(frames):
            ...
            if (
                ...
                and identical > self.maximum_identical_frame_run
            ):
            ...
            if accepted:
                ...
                previous_index, previous_timestamp = (
                    frame.frame_index,
