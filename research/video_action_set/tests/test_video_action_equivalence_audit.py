...
def test_raw_action_gaps_do_not_trigger_material_governed_utility_status(
    tmp_path: Path,
) -> None:
    summary = run_finalize_audit(tmp_path, tmp_path / "reachability")
    assert (
...
def test_unavailable_replay_is_not_treated_as_reachability_failure(
    tmp_path: Path,
) -> None:
    run_finalize_audit(tmp_path, tmp_path / "reachability")
...
def test_verified_reachability_tile_does_not_imply_replay_success(
    tmp_path: Path,
) -> None:
    run_finalize_audit(tmp_path, tmp_path / "reachability")
    replay = json.loads(
        (tmp_path / "reachability-replay-summary.json").read_text(encoding="utf-8")
    )
...
def test_visual_branch_is_neither_closed_nor_promoted(tmp_path: Path) -> None:
    ...
    recommendation = json.loads(
        (tmp_path / "visual-branch-recommendation.json").read_text(encoding="utf-8")
    )
    assert (
        recommendation["visual_branch_recommendation"]
        == "undetermined_due_to_missing_artifacts"
    )
