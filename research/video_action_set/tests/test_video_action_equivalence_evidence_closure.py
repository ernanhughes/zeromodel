...
def test_aggregate_metrics_do_not_imply_per_observation_evidence(
    tmp_path: Path,
) -> None:
    run_audit_evidence_closure(tmp_path)
    closure = __import__("json").loads(
        (tmp_path / "evidence-closure.json").read_text(encoding="utf-8")
    )
    provider = next(
        item for item in closure["providers"] if item["provider_id"] == "stage3-v1"
    )
...
def test_current_branch_sha_is_not_substituted_for_historical_source_identity(
    tmp_path: Path,
) -> None:
    run_audit_evidence_closure(tmp_path)
    inventory = __import__("json").loads(
...
def test_empty_reproduction_command_cannot_produce_verified_reproducibility(
    tmp_path: Path,
) -> None:
    run_audit_evidence_closure(tmp_path)
    inventory = __import__("json").loads(
    ...
    provider = next(
        item for item in inventory["providers"] if item["provider_id"] == "system-b-v2"
    )
    assert provider["reproducibility_status"] == "not_tested"


def test_canonical_instrument_reproducibility_does_not_imply_evaluation_scores(
    tmp_path: Path,
) -> None:
    run_audit_evidence_closure(tmp_path)
    inventory = __import__("json").loads(
    ...
    provider = next(
        item for item in inventory["providers"] if item["provider_id"] == "stage3-v3-b3"
    )
...
def test_invalid_v2_historical_artifacts_are_not_relabelled_reproducible(
    tmp_path: Path,
) -> None:
    run_audit_evidence_closure(tmp_path)
    inventory = __import__("json").loads(
    ...
    provider = next(
        item for item in inventory["providers"] if item["provider_id"] == "stage3-v2"
    )
...
def test_policy_row_action_map_still_covers_entire_policy() -> None:
    mapping = build_policy_row_action_map(
