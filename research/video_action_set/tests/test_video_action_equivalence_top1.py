...
def _results(tmp_path: Path) -> list[dict[str, object]]:
    ...
    return json.loads(
        (tmp_path / "top1-action-results.json").read_text(encoding="utf-8")
    )
...
def test_row_to_action_mapping_uses_exact_policy_artifact() -> None:
    ...
    assert (
        policy_action_for_row(
            ...
            row_action_map=mapping,
            policy_artifact_id=mapping[0]["policy_artifact_id"],
        )
        == "STAY"
    )
...
def test_canonical_diagnostics_are_not_labelled_benchmark_utility(
    tmp_path: Path,
) -> None:
    results = _results(tmp_path)
