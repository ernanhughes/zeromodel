...
def test_artifact_comparison_detects_changed_sample_size(tmp_path: Path) -> None:
    ...
    _copy_artifacts(
        Path("docs/results/video-discriminative-local-evidence-v1"), expected
    )
    _copy_artifacts(Path("docs/results/video-discriminative-local-evidence-v1"), actual)
    manifest = json.loads(
        (actual / "benchmark-manifest.json").read_text(encoding="utf-8")
    )
    manifest["metadata"]["nonprototype_row_sample_size"] = 99
    (actual / "benchmark-manifest.json").write_text(
    ...
    assert (
        next(
            ...
            if item["artifact"] == "benchmark-manifest.json"
        )["status"]
        == "semantic_mismatch"
    )


def test_artifact_comparison_detects_changed_family_definition_and_sampling(
    tmp_path: Path,
) -> None:
    expected = tmp_path / "expected"
    actual = tmp_path / "actual"
    ...
    split_manifest = json.loads(
        (actual / "split-manifest.json").read_text(encoding="utf-8")
    )
    split_manifest["observation_membership"]["architecture_selection_benign"] = (
        split_manifest["observation_membership"]["architecture_selection_benign"][:-1]
    )
    split_manifest["clip_membership"]["architecture_selection_benign"] = split_manifest[
        "clip_membership"
    ]["architecture_selection_benign"][:-1]
    (actual / "split-manifest.json").write_text(
    ...
    assert (
        next(
            item
            for item in comparison["artifacts"]
            if item["artifact"] == "split-manifest.json"
        )["status"]
        == "semantic_mismatch"
    )


def test_artifact_comparison_detects_changed_mask_digest_and_stale_files(
    tmp_path: Path,
) -> None:
    expected = tmp_path / "expected"
    actual = tmp_path / "actual"
    ...
    mask_manifest = json.loads(
        (actual / "mask-manifest.json").read_text(encoding="utf-8")
    )
    mask_manifest["mask_spec_digest"] = "sha256:deadbeef"
    (actual / "mask-manifest.json").write_text(
        json.dumps(mask_manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
...
def test_prototype_closure_reports_missing_rows_and_zero_stability() -> None:
    ...
    freeze = bench._freeze_regions_and_masks(
        benchmark,
        output_dir=Path(
            "docs/results/video-discriminative-local-evidence-v1/measurement-audit/test-scratch"
        ),
    )
    closure = bench.validate_prototype_and_development_closure(
...
def test_exact_frame_audit_shows_current_code_has_nonzero_evidence(
    tmp_path: Path,
) -> None:
...
def test_audit_pre_final_v1_marks_stale_artifacts_invalid() -> None:
    with pytest.raises(SystemExit, match="invalid_multiple_failures"):
        bench._run_audit_pre_final_v1(
            Path("docs/results/video-discriminative-local-evidence-v1")
        )
    ruling = bench._load_v1_ruling(
        Path("docs/results/video-discriminative-local-evidence-v1")
    )
