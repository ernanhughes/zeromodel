...
def test_representation_audit_freezes_expected_v2_failures(tmp_path: Path) -> None:
    ...
    try:
        ...
        raise AssertionError(
            "representation audit should fail nonzero for frozen v2 defects"
        )
    ...
    assert (
        payload["mask_separation_summary"]["rows_with_positive_stable_informative_mass"]
        == 112
    )
    assert (
        payload["mask_separation_summary"][
            "rows_with_positive_separation_weighted_mass"
        ]
        == 0
    )
    assert payload["distributed_competitor_cover_row_count"] == 112
    assert (
        payload["architecture_a_conformance"]["ruling"]
    ...
    for row_id, mask in sorted(freeze["masks"].items()):
        ...
        rows.append(
            (
                ...
                int(
                    (
                        (arrays["row_informative"] > 0.0)
                        & (arrays["separation"] <= 0.0)
                    ).sum()
                ),
...
def test_strict_exact_tie_check_rejects_lexical_ties(tmp_path: Path) -> None:
    ...
    ranked = next(
        iter(
            bench._canonical_rankings_for_architecture(
                benchmark=benchmark,
                freeze=freeze,
                architecture_id="B",
    ...
    assert (
        bench._strict_zero_margin_exact_eligibility(ranked[0], calibration=calibration)
        is False
    )
