...
def test_v3_freeze_preserves_counts_and_pairwise_universe(tmp_path: Path) -> None:
    ...
    assert (
        pairwise["same_action_pair_count"] + pairwise["different_action_pair_count"]
        == 6216
    )
...
def test_v3_ids_are_disjoint_from_v2_ids() -> None:
    ...
    assert {record.observation_id for record in v2.records}.isdisjoint(
        {record.observation_id for record in v3.records}
    )
    assert {record.clip_id for record in v2.records}.isdisjoint(
        {record.clip_id for record in v3.records}
    )
    assert {record.frame_id for record in v2.records}.isdisjoint(
        {record.frame_id for record in v3.records}
    )
