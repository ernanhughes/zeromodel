...
def test_committed_v3_audit_reports_outcome_b() -> None:
    summary = bench._load_json(
        bench.OUTPUT_DIR_V3 / "architecture-self-retrieval-status.json"
    )
    assert summary["shared_instrument_valid"] is True
    assert summary["stop_outcome"] == "Outcome B"
    assert (
        summary["architectures"]["B3"]["self_retrieval_status"]
        == "eligible_self_retrieval"
    )
    assert summary["architectures"]["B3"]["canonical_top1_count"] == 112
    assert (
        summary["architectures"]["A3"]["self_retrieval_status"]
        == "ineligible_self_retrieval"
    )
    assert (
        summary["architectures"]["C3"]["self_retrieval_status"]
        == "ineligible_self_retrieval"
    )
    assert (
        summary["architectures"]["D3"]["self_retrieval_status"]
        == "ineligible_self_retrieval"
    )
...
def test_committed_v3_tie_safety_and_pairwise_symmetry_hold() -> None:
    ...
    equivalence = bench._load_json(
        bench.OUTPUT_DIR_V3 / "provider-equivalence-results.json"
    )
