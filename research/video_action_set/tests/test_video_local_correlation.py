...
def test_local_correlation_provider_separates_identical_pixels_across_clips_in_cache_identity() -> (
    None
):
    provider = _provider()
...
def test_local_correlation_provider_rejects_geometry_mismatch() -> None:
    ...
    with pytest.raises(VPMValidationError, match="geometry"):
        provider.read(
            ImageObservation(np.zeros((8, 8), dtype=np.uint8), source_id="bad")
        )
...
def test_video_benchmark_generation_is_deterministic() -> None:
    ...
    assert [case.source.manifest().manifest_id for case in left] == [
        case.source.manifest().manifest_id for case in right
    ]
...
def test_video_benchmark_reordered_case_has_distinct_identity() -> None:
    ...
    assert (
        cases["video-final-negative-reordered"].source.manifest().manifest_id
        != cases["video-final-benign-exact"].source.manifest().manifest_id
    )
