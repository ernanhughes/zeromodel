...
def test_reachable_rows_stay_within_universe(tmp_path: Path) -> None:
    ...
    assert all(
        dest in sources for edge in tile["edges"] for dest in edge["reachable_row_ids"]
    )
...
def test_aggregate_sequence_evidence_blocks_replay(tmp_path: Path) -> None:
    ...
    assert (
        payload["stage3_v1_classification"]
        == "sequence_metadata_without_visual_beliefs"
    )


def test_missing_visual_sets_cannot_be_reconstructed_from_aggregate_metrics(
    tmp_path: Path,
) -> None:
    payload = run_replay_reachability(tmp_path)
