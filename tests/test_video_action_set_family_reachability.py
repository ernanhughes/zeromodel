...
def _outcome(
    rows: list[str], row_actions: dict[str, str], *, status: str | None = None
) -> dict[str, object]:
...
def test_valid_transformations_accept_bounds_and_regenerate_bytes() -> None:
    ...
    upper = {
        **lower,
        ...
        "scale_percent": 105,
        "offset": 5,
        "occlusion": {"top": 2, "left": 2, "height": 2, "width": 3, "value": 64},
    }
    upper["parameter_digest"] = benchmark._sha256(
        {key: value for key, value in upper.items() if key != "parameter_digest"}
    )
    interior = {
        **lower,
        "dx": 0,
        ...
        "scale_percent": 100,
        "offset": 3,
        "occlusion": {"top": 1, "left": 1, "height": 2, "width": 3, "value": 64},
    }
    interior["parameter_digest"] = benchmark._sha256(
        {key: value for key, value in interior.items() if key != "parameter_digest"}
    )
...
def test_conflicting_splice_rejects_same_action_and_records_contributions() -> None:
    ...
    plan = next(
        ...
        if item["mutation_kind"] == "conflicting_action_splice"
    )
    ...
    same_action_row = next(
        ...
        if row_actions[row_id] == row_actions[plan["source_row_id"]]
        and row_id != plan["source_row_id"]
    )
    with pytest.raises(VPMValidationError):
...
def test_critical_corruption_rejects_bad_coordinates_and_stale_digest() -> None:
    ...
    output, trace = benchmark._apply_critical_corruption(
        source, benchmark._critical_coordinate_manifest()
    )
    assert trace["changed_pixel_count"] == 4
    record = {
        "pixels": output,
        "observation_pixel_digest": benchmark._array_digest(source),
...
def test_temporal_families_materialize_real_sequence_events() -> None:
    ...
    reordered = next(
        item for item in plans if item["mutation_kind"] == "reordered_frames"
    )
    reordered_records = benchmark._materialize_plan(reordered, identity, tile)
    assert [
        record["metadata"]["original_frame_index"] for record in reordered_records
    ] == [1, 0, 2, 3]
    stale = next(
    ...
    assert (
        stale_event["original_destination_digest"] != stale_event["replacement_digest"]
    )
    assert (
        stale_records[1]["observation_pixel_digest"]
        == stale_records[0]["observation_pixel_digest"]
    )
    gap = next(
        item
        for item in plans
        if item["mutation_kind"] == "declared_gap_or_unknown_action"
    )
...
def test_information_controls_are_byte_identical_and_denominator_excluded() -> None:
    ...
    tampered[1]["observation_pixel_digest"] = benchmark._array_digest(
        tampered[1]["pixels"]
    )
    assert (
        benchmark.validate_control_episode_records(tampered)
...
def test_reachability_composition_mutation_failure_codes() -> None:
    ...
    for item in mutated["edges"]:
        if item["source_row_id"] == source and item["action_id"] == row_actions[source]:
            item["reachable_row_ids"] = [
                row for row in item["reachable_row_ids"] if row != destination
            ]
            break
    assert (
        benchmark.validate_reachability_trace(
            trace,
            semantic_outcome=_outcome([destination], row_actions),
            previous_state={"status": "resolved", "candidate_rows": (source,)},
...
def test_ambiguous_reachability_never_uses_lexical_winner() -> None:
    _identity, row_ids, row_actions = _identity_rows_actions()
    tile = benchmark._load_reachability_tile(REPO_ROOT)
    conflicting = [row for row in row_ids if row_actions[row] == "LEFT"][:1] + [
        row for row in row_ids if row_actions[row] == "RIGHT"
    ][:1]
    trace = benchmark.compose_reachability_trace(
        frame_id="frame-0",
        semantic_outcome=_outcome(
            conflicting, row_actions, status="conflicting_action_tie"
        ),
        previous_state=None,
        reachability_tile=tile,
        row_actions=row_actions,
    ...
    unanimous = benchmark.compose_reachability_trace(
        frame_id="frame-0",
        semantic_outcome=_outcome(
            same_action, row_actions, status="action_unanimous_tie"
        ),
        previous_state=None,
        reachability_tile=tile,
        row_actions=row_actions,
