...
def _legacy_secondary_row_for_splice(
    row_ids: list[str], row_actions: dict[str, str], source_row_id: str
) -> str:
    source_action = row_actions[source_row_id]
    source_tank, source_target, source_cooldown = benchmark.parse_state_row_id(
        source_row_id
    )
...
def _first_splice_plan() -> dict[str, object]:
    ...
    return next(
        plan
        for plan in benchmark._episode_plans_for_split(
...
def test_legacy_row_band_conflicting_splice_collided_with_canonical_universe() -> None:
    ...
    for source_row_id in row_ids[:28]:
        secondary_row_id = _legacy_secondary_row_for_splice(
            row_ids, row_actions, source_row_id
        )
        ...
        if digest in canonical_index:
            collisions.append(
                (source_row_id, secondary_row_id, canonical_index[digest])
            )
    assert len(collisions) == 28


def test_repaired_conflicting_splice_selection_frames_do_not_decode_as_canonical_valid() -> (
    None
):
    records = benchmark._materialize_records("selection", REPO_ROOT)
    canonical_digests = set(
        benchmark.canonical_observation_universe()["digest_to_rows"]
    )
    splice_records = [
        record for record in records if record["family"] == "conflicting_action_splice"
    ]

    assert len(splice_records) == 112
    assert all(
        record["observation_pixel_digest"] not in canonical_digests
        for record in splice_records
    )
    assert {
        benchmark.validate_materialized_family_record(record)
        for record in splice_records
    } == {"ok"}
    for record in splice_records:
...
def test_conflicting_splice_rejects_same_action_and_missing_or_overlapping_target_evidence() -> (
    None
):
    _identity, row_ids, row_actions = _identity_rows_actions()
    ...
    same_action_row = next(
        row_id
        for row_id in row_ids
        if row_id != source_row_id and row_actions[row_id] == row_actions[source_row_id]
    )
    no_target_row = "tank=0|target=none|cooldown=0"
    visible_conflict_row = next(
        row_id
        for row_id in row_ids
        if benchmark.parse_state_row_id(row_id)[1] is not None
        and row_actions[row_id] != row_actions[no_target_row]
    )
...
def test_changing_either_splice_source_changes_trace_identity() -> None:
    ...
    changed_secondary = next(
        ...
        and benchmark.parse_state_row_id(row_id)[1]
        not in {None, primary_target, secondary_target}
    ...
    assert benchmark._array_digest(first_output) != benchmark._array_digest(
        second_output
    )
    assert benchmark._array_digest(first_output) != benchmark._array_digest(
        third_output
...
def test_information_controls_have_hidden_history_ambiguity_and_no_visible_leak() -> (
    None
):
    identity, row_ids, row_actions = _identity_rows_actions()
    plan = next(
        item
        for item in benchmark._episode_plans_for_split(
            identity, "selection", row_ids, row_actions
        )
    ...
    assert {record["observation_pixel_digest"] for record in records} == {
        records[0]["observation_pixel_digest"]
    }
    assert {record["metadata"]["hidden_source_history_id"] for record in records} == {
        history["hidden_history_id"]
        for history in plan["family_intervention"]["control_group"][
            "hidden_source_histories"
        ]
    }
    assert len(
        {record["metadata"]["hidden_source_label_digest"] for record in records}
    ) == len(records)
    assert all(
        record["metadata"]["provider_visible_fields"]
        == [
            "pixels",
            "shape",
            "raw_digest",
    ...
    assert (
        len({record["metadata"]["provider_observation_digest"] for record in records})
        == 1
    )
    assert (
        len(
            {
                benchmark._provider_observation_digest(
                    benchmark.provider_observation_for_record(record).to_descriptor()
                )
                for record in records
            }
        )
    ...
    assert {record["episode_disposition"] for record in records} == {
        "information_theoretic_control"
    }
    assert {record["frame_disposition"] for record in records} == {
        "information_theoretic_control"
    }
    assert {record["denominator_class"] for record in records} == {
        "excluded_information_control"
    }


def test_information_control_validator_rejects_hidden_label_collapse_and_visible_leak() -> (
    None
):
    identity, row_ids, row_actions = _identity_rows_actions()
    plan = next(
    ...
    for record in collapsed[1:]:
        record["metadata"]["hidden_source_history_id"] = collapsed[0]["metadata"][
            "hidden_source_history_id"
        ]
        record["metadata"]["hidden_source_label_digest"] = collapsed[0]["metadata"][
            "hidden_source_label_digest"
        ]
    assert (
        benchmark.validate_control_episode_records(collapsed)
    ...
    assert (
        benchmark.validate_control_episode_records(leaked)
...
def test_episode_dispositions_and_final_observation_provenance_are_explicit() -> None:
    ...
    temporal_plan = next(
        item for item in plans if item["mutation_kind"] == "stale_repeated_frame"
    )
    ...
    assert {record["episode_family"] for record in temporal_records} == {
        "temporal_negative"
    }
    assert {record["episode_disposition"] for record in temporal_records} == {
        "distinguishable_temporal_invalid"
    }
    assert "stale_repeated_frame_payload" in {
        record["frame_disposition"] for record in temporal_records
    }

    final_plan = next(
        item
        for item in benchmark._episode_plans_for_split(
            identity, "final", row_ids, row_actions
        )
        if item["family_label"] == "information_control"
    )
...
def test_information_controls_use_real_convergent_causal_histories() -> None:
    ...
    reconstructed = {
        benchmark._reconstructed_control_causal_tuple_digest(history)
        for history in histories
    }
    assert len(reconstructed) >= 2
    assert {history["resulting_row_id"] for history in histories} == {
        control_group["current_row_id"]
    }
    assert {record["metadata"]["control_current_row_id"] for record in records} == {
        control_group["current_row_id"]
    }
    assert len({record["observation_pixel_digest"] for record in records}) == 1
    assert all(
        history["actual_executed_action"] in benchmark.ACTIONS for history in histories
    )
...
def test_information_control_same_causal_tuple_with_distinct_ids_is_rejected() -> None:
    ...
    for index, record in enumerate(collapsed):
        ...
        record["metadata"]["hidden_source_label_digest"] = history[
            "hidden_source_label_digest"
        ]
    assert (
        benchmark.validate_control_episode_records(collapsed)
...
def test_control_provider_observation_descriptor_is_identical_across_group() -> None:
    ...
    assert (
        len(
            {
                benchmark._provider_observation_digest(descriptor)
...
def test_control_actual_provider_source_id_leak_is_rejected() -> None:
    ...
    leaked[0]["metadata"]["provider_observation_digest"] = (
        benchmark._provider_observation_digest(descriptor)
    )
...
def test_operation_chain_replays_every_materialized_family() -> None:
    ...
    for family in (
        ...
        replay = benchmark.replay_observation_operation_chain(
            record["metadata"]["observation_operation_chain"]
        )
    ...
    temporal_records = [
        record for record in records if record["episode_family"] == "temporal_negative"
    ]
    stale = next(
        record
        for record in temporal_records
        if record["frame_disposition"] == "stale_repeated_frame_payload"
    )
    impossible = next(
        record
        for record in temporal_records
        if record["frame_disposition"] == "unreachable_destination_frame_payload"
    )
    gap = next(
        record
        for record in temporal_records
        if record["frame_disposition"] == "declared_gap_or_unknown_action"
    )

    assert "stale_repeat_replace" in [
        op["operation"]
        for op in stale["metadata"]["observation_operation_chain"]["operations"]
    ]
    assert "impossible_transition_replace" in [
        op["operation"]
        for op in impossible["metadata"]["observation_operation_chain"]["operations"]
    ]
    assert "emit_typed_gap_event" in [
        op["operation"]
        for op in gap["metadata"]["observation_operation_chain"]["operations"]
    ]
...
def test_laundered_stale_repeat_operation_removal_is_rejected() -> None:
    ...
    chain["operation_chain_digest"] = benchmark._sha256(
        {key: value for key, value in chain.items() if key != "operation_chain_digest"}
    )
    mutated["metadata"]["observation_operation_chain"] = chain

    assert (
        mutated["observation_pixel_digest"]
        == stale["metadata"]["stale_repeat"]["replacement_digest"]
    )
    assert original_digest != mutated["observation_pixel_digest"]
    assert (
        benchmark.validate_observation_operation_chain(mutated)
...
def test_conflicting_splice_final_visible_targets_imply_distinct_actions() -> None:
    ...
    for record in splice_records:
        ...
        evidence = benchmark._final_visible_target_action_evidence(
            replay["pixels"], row_actions
        )
        trace = record["metadata"]["family_intervention_trace"]
        assert evidence["conflicting_action_evidence_present"] is True
        assert len(evidence["visible_action_set"]) >= 2
        assert (
            trace["visible_action_evidence_digest"]
            == evidence["visible_action_evidence_digest"]
        )
...
def test_action_unanimous_two_target_splice_is_rejected() -> None:
    ...
    assert (
        benchmark._splice_pair_has_final_visible_action_conflict(
            primary, secondary, row_actions
        )
        is False
    )
...
def test_exact_frame_disposition_reconstruction_for_temporal_families() -> None:
    ...
    plans = [
        ...
        if plan["family_label"] == "temporal_negative"
    ]
    ...
    for kind, plan in by_kind.items():
        ...
        reconstructed = [
            benchmark.expected_frame_disposition(
                "temporal_negative", kind, int(record["sequence_number"]), intervention
            )
            for record in records
        ]
...
def test_bounded_transformed_valid_universe_closes_frame_invalid_outputs() -> None:
    ...
    assert universe["transformed_valid_digest_count"] == len(
        transformed["digest_index"]
    )
    assert (
        universe["policy_artifact_id"]
        == benchmark.compile_policy_artifact().artifact_id
    )
    assert universe["renderer_identity"]["renderer_identity_digest"].startswith(
        "sha256:"
    )
    assert all(
        record["observation_pixel_digest"] not in canonical for record in invalid
    )
    assert all(
        record["observation_pixel_digest"] not in transformed_only for record in invalid
    )
