...
def _rows(count: int = 112) -> list[tuple[str, float]]:
    return [
        (f"row-{index:03d}", max(0.0, 1.0 - index / 200.0)) for index in range(count)
    ]


def _row_actions(prefix: str = "row") -> dict[str, str]:
    return {
        f"{prefix}-{index:03d}": ("LEFT" if index % 2 == 0 else "RIGHT")
...
def test_complete_row_evidence_preserves_ties_without_semantic_uniqueness() -> None:
    ...
    assert evidence.ranking.ranked_row_ids == tuple(
        sorted(row_id for row_id, _score in rows)
    )


def test_v2_digest_separates_quantized_identity_from_raw_diagnostics() -> None:
    rows = _rows()
    base = build_complete_row_evidence(
        row_scores=rows,
        policy_artifact_id="policy",
        provider_id="P1",
        provider_version="v1",
    ...
    same_bin = build_complete_row_evidence(
        row_scores=tweaked,
        policy_artifact_id="policy",
        provider_id="P1",
        provider_version="v1",
    ...
    different_bin = build_complete_row_evidence(
        row_scores=changed,
        policy_artifact_id="policy",
        provider_id="P1",
        provider_version="v1",
    ...
    assert (
        base.quantized_score_vector_digest
        != different_bin.quantized_score_vector_digest
    )
...
def test_semantic_outcome_unique_top_row_resolves_row_and_action() -> None:
    ...
    outcome = build_semantic_top_set_outcome(
        evidence=evidence, row_action=_row_actions()
    )
...
def test_semantic_outcome_renaming_tied_rows_preserves_semantic_class() -> None:
    rows = [
        (f"alt-{index:03d}", score) for index, (_row_id, score) in enumerate(_rows())
    ]
...
def test_semantic_outcome_rejects_changed_top_row_action() -> None:
    ...
    payload = build_semantic_top_set_outcome(
        evidence=evidence, row_action=actions
    ).to_dict()
    payload["top_row_actions"][1]["action_id"] = "RIGHT"
    with pytest.raises(VPMValidationError):
        semantic_top_set_outcome_from_dict(
            payload, evidence=evidence, row_action=actions
        )
...
def test_semantic_outcome_digest_ignores_raw_score_changes_inside_quantized_bin() -> (
    None
):
    actions = _row_actions()
    baseline_evidence = build_complete_row_evidence(
        row_scores=_rows(),
        policy_artifact_id="policy",
        provider_id="P1",
        provider_version="v1",
    ...
    tweaked_evidence = build_complete_row_evidence(
        row_scores=tweaked,
        policy_artifact_id="policy",
        provider_id="P1",
        provider_version="v1",
    ...
    baseline = build_semantic_top_set_outcome(
        evidence=baseline_evidence, row_action=actions
    )
    same_bin = build_semantic_top_set_outcome(
        evidence=tweaked_evidence, row_action=actions
    )
    assert (
        baseline_evidence.raw_score_diagnostic_digest
        != tweaked_evidence.raw_score_diagnostic_digest
    )
    assert (
        baseline_evidence.quantized_score_vector_digest
        == tweaked_evidence.quantized_score_vector_digest
    )
...
def test_semantic_outcome_constructor_rejects_duplicate_top_rows_and_foreign_digest() -> (
    None
):
    outcome = build_semantic_top_set_outcome(
        evidence=build_complete_row_evidence(
