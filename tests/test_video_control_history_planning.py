...
def test_transition_identity_and_causal_tuple_goldens() -> None:
    ...
    assert control_histories.transition_result_digest(
        source_row_id, action, 0, source_row_id
    ) == ("sha256:0f1816adedc21735c89854d8e91f922de335892b7fe97def1941f59c2c2a3bca")
    assert (
        canonical_sha256(causal_tuple)
        == FIRST_CANDIDATE["normalized_causal_tuple_digest"]
    )
...
def test_control_history_enumeration_selection_and_reconstruction_are_frozen() -> None:
    ...
    assert (
        control_histories.control_source_rows(row_ids, count=3, frame_count=4)
        == CONTROL_SOURCE_ROWS
    )
    candidates = control_histories.grounded_control_histories_for_current_row(
        CONTROL_SOURCE_ROWS[0], row_ids
    )
    assert len(candidates) == 6
    assert candidates[0] == FIRST_CANDIDATE
    assert (
        canonical_sha256(candidates)
        == "sha256:fe5bd93b70a20d87c12f0e543acdc56f0b2372e3ebdf68ae2d06df6cbe30e8fe"
    )
    ...
    assert (
        ...
        == "sha256:77e821ed82496125338322aa62a1c61321c54c68acb8d5b04a14ac18903183e8"
    )
    assert (
        control_histories.reconstructed_control_causal_tuple_digest(selected[0])
        == SELECTED_HISTORY_IDS[0]
    )
...
def test_benchmark_control_history_aliases_are_direct() -> None:
    ...
    assert (
        benchmark._transition_input_digest is control_histories.transition_input_digest
    )
    assert (
        benchmark._transition_result_digest
        is control_histories.transition_result_digest
    )
    assert (
        benchmark._normalized_control_causal_tuple
        is control_histories.normalized_control_causal_tuple
    )
    assert (
    ...
    assert (
        benchmark._select_grounded_control_histories
        is control_histories.select_grounded_control_histories
    )
