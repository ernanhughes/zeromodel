...
def _summary(plans: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "family_counts": dict(
            sorted(Counter(str(plan["family_label"]) for plan in plans).items())
        ),
        "mutation_counts": dict(
            sorted(
                Counter(
                    str(plan["mutation_kind"])
                    for plan in plans
                    if plan.get("mutation_kind") is not None
                ).items()
            )
        ),
...
def test_derived_seed_and_representative_episode_plan_are_frozen() -> None:
    ...
    assert (
        seed["seed_digest"]
        == "sha256:4d09890ab6c6cec5916e4d3a8872fd08016dcf7eebce4586f7965db8ce419a40"
    )
    assert seed["seed_int64"] == 5551118694820007621
    assert plan["episode_id"] == "selection:valid:3b017d7e360a1086"
    assert (
        plan["plan_digest"]
        == "sha256:711d180c93547b5f6f1489cccaf5648ffb8ab7af3e0d55c8cc9cd8416d11ff0e"
    )
    ...
    assert [
        frame["transformation_parameter_digest"] for frame in plan["frame_plans"]
    ] == [
...
def test_split_episode_plan_collections_match_pre_extraction_goldens(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fail(*_args: object, **_kwargs: object) -> None:
        raise AssertionError(
            "episode planning must not materialize pixels or provider scores"
        )
    ...
    identity, row_ids, row_actions = _identity_rows_actions()
    plans_by_split = {
        split: episode_planning.episode_plans_for_split(
    ...
    assert {
        split: _summary(plans) for split, plans in plans_by_split.items()
    } == EXPECTED_SPLIT_SUMMARIES
    assert canonical_sha256(
        {
            split: [plan["plan_digest"] for plan in plans]
            for split, plans in plans_by_split.items()
        }
    ) == ("sha256:a56aef694069a5bc46639834e6d323866a35b701e1085c743b4b080918a0bced")
    assert all(
        not _contains_materialized_payload(plan)
        for plans in plans_by_split.values()
        for plan in plans
    )
    assert all(
        plan["final_observation_provenance"]["observation_payload_included"] is False
        for plan in plans_by_split["final"]
    )


def test_episode_plan_validation_rejects_lineage_and_identity_tampering() -> None:
    identity, row_ids, row_actions = _identity_rows_actions()
    plan = episode_planning.episode_plans_for_split(
        identity, "selection", row_ids, row_actions
    )[0]
    tampered = deepcopy(plan)
    tampered["seed_lineage"]["concrete_episode_seed"]["seed_digest"] = (
        "sha256:" + "0" * 64
    )
    ...
    with pytest.raises(VPMValidationError, match="duplicate concrete episode identity"):
        episode_planning.validate_episode_plan_collection(
            identity, {"selection": [plan, plan]}, row_actions
        )
...
def test_benchmark_episode_planning_aliases_are_direct() -> None:
    ...
    assert (
        benchmark._final_observation_provenance
        is episode_planning.final_observation_provenance
    )
    ...
    assert (
        benchmark._episode_plans_for_split is episode_planning.episode_plans_for_split
    )
    ...
    assert (
        ...
        is episode_planning.validate_episode_plan_collection
    )
    ...
    assert (
        benchmark._grounded_control_history
        is control_histories.grounded_control_history
    )
