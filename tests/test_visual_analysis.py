...
def _decision(
    *, accepted: bool, nearest_row_id: str | None, nearest_score: float | None
...
def _trace(
    observation_id, family_id, expected, row, action, top1_row, top1_action, score
):
...
def test_trace_point_contract_uses_canonical_dispositions() -> None:
    ...
    assert (
        ...
        and not accept.expected_reject
        and not accept.information_theoretic_control
    )
    assert (
        reject.expected_reject
        and not reject.expected_accept
        and not reject.information_theoretic_control
    )
    assert (
        control.information_theoretic_control
        and not control.expected_accept
        and not control.expected_reject
    )


def test_global_threshold_curve_separates_ranking_from_thresholding_and_excludes_controls() -> (
    None
):
    points = trace_points(
        (
            ...
            _trace(
                "d", "control", IMPOSSIBILITY_CONTROL, "r3", "stay", "r3", "stay", 0.95
            ),
        )
    )
...
def test_analysis_marks_curves_as_exploratory() -> None:
    traces = {
        "B": (_trace("a", "x", EXPECTED_ACCEPT, "r1", "left", "r1", "left", 0.8),),
        "D": (_trace("a", "x", EXPECTED_ACCEPT, "r1", "left", "r2", "left", 0.9),),
    }
