...
class DeterministicVisualAddressProvider:
    ...
    def __init__(self, reader: VisualSignReader, *, source_scope: Optional[str] = None) -> None:
        self.reader = reader
    ...
    def read(self, observation: ImageObservation) -> VisualAddressDecision:
        ...
        return self._map(observation, self.reader.read(observation.pixels))

    def _map(self, observation: ImageObservation, decision: VisualDecision) -> VisualAddressDecision:
        checks = []
    ...
    def __post_init__(self) -> None:
        ...
        if self.accepted:
            if self.policy is None or not self.address.accepted:
                raise VPMValidationError("accepted visual policy decision needs both traces")
...
class VisualPolicyReader:
    def __init__(self, provider: VisualAddressProvider, policy_lookup: VPMPolicyLookup) -> None:
        contract = provider.contract()
        if contract.policy_artifact_id != policy_lookup.artifact.artifact_id:
            raise VPMValidationError("visual provider targets policy %s, not %s" % (
    ...
    def read(self, observation: ImageObservation) -> VisualPolicyDecision:
        address = self.provider.read(observation)
        observed = (
            address.provider_kind, address.provider_version, address.score_semantics,
            address.address_artifact_id, address.calibration_artifact_id,
            address.policy_artifact_id,
        )
        expected = (
            self._contract.provider_kind, self._contract.provider_version,
            self._contract.score_semantics, self._contract.address_artifact_id,
            self._contract.calibration_artifact_id, self._contract.policy_artifact_id,
        )
        if observed != expected:
            raise VPMValidationError("visual address decision violates provider contract")
        if not address.accepted:
            return VisualPolicyDecision(False, address.reason, address)
        policy = self.policy_lookup.read(str(address.matched_row_id))
        return VisualPolicyDecision(True, "accepted", address, policy)
