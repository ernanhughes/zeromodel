...
from .visual_address_manifest import PrototypeBinding, VisualAddressManifest
from .visual_encoder import EncoderManifest, FrozenVisualEncoder


VECTOR_ADDRESS_READER_VERSION = "zeromodel-vector-address-reader/v1"
VECTOR_CALIBRATION_VERSION = "zeromodel-vector-calibration/v1"
LINEAR_PROBE_VERSION = "zeromodel-linear-probe/v1"
...
def _representation_digest(vector: np.ndarray, representation_spec_digest: str) -> str:
    item = np.ascontiguousarray(vector, dtype=">f4")
    payload = (
        b"zeromodel.vector-representation.v1\0"
...
class VectorCalibration:
    ...
    def __post_init__(self) -> None:
        thresholds = {str(key): float(value) for key, value in self.acceptance_thresholds.items()}
        margins = {str(key): float(value) for key, value in self.ambiguity_margins.items()}
        object.__setattr__(self, "acceptance_thresholds", MappingProxyType(thresholds))
        object.__setattr__(self, "ambiguity_margins", MappingProxyType(margins))
        object.__setattr__(self, "metadata", _freeze(self.metadata))
        if self.version != VECTOR_CALIBRATION_VERSION:
            raise VPMValidationError("unsupported vector calibration version")
        if not thresholds or set(thresholds) != set(margins):
            raise VPMValidationError("calibration thresholds and margins require identical rows")
        if any(not np.isfinite(value) for value in tuple(thresholds.values()) + tuple(margins.values())):
            raise VPMValidationError("calibration thresholds and margins must be finite")
        if not np.isfinite(self.calibration_quantile) or not (0.0 <= self.calibration_quantile <= 1.0):
...
class VectorAddressBuild:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "prototype_action_ids", tuple(str(value) for value in self.prototype_action_ids))
        ...
        if self.matrix_blob.blob_id != self.manifest.matrix_blob_id:
            raise VPMValidationError("address manifest does not reference supplied matrix blob")
        if self.calibration.digest != self.manifest.calibration_artifact_id:
            raise VPMValidationError("address manifest does not reference supplied calibration")
...
def _validate_labels(
    ...
    *,
    ...
    if not (len(rows) == len(actions) == len(observations) == vectors.shape[0]):
        raise VPMValidationError("%s vectors and labels must have identical length" % label)
...
def _select_medoid_indices(vectors: np.ndarray, row_ids: Sequence[str]) -> Tuple[int, ...]:
    selected = []
    for row_id in sorted(set(row_ids)):
        ...
        best_local = max(range(len(indices)), key=lambda index: (float(totals[index]), -indices[index]))
...
def _calibrate_prototypes(
    ...
    *,
    ...
    for vector, row_id, action_id in zip(
        calibration_vectors, calibration_rows, calibration_actions
    ):
        similarities = prototypes @ vector
        same = [
            index for index, candidate_row in enumerate(prototype_rows_tuple)
            if candidate_row == row_id
        ]
        if not same:
            raise VPMValidationError("calibration row has no matching prototype: %s" % row_id)
        correct_score = max(float(similarities[index]) for index in same)
        conflicts = [
            index for index, candidate_action in enumerate(prototype_actions_tuple)
            if candidate_action != action_id
        ]
        conflict_score = (
            max(float(similarities[index]) for index in conflicts)
            if conflicts
            else -1.0
        )
    ...
    margins = {
        row_id: _lower_quantile(margins_by_row[row_id], quantile)
        for row_id in sorted(expected_rows)
    }
    return VectorCalibration(
        ...
        metadata={"prototype_strategy": strategy, "score": "cosine_similarity"},
    )
...
def build_vector_address(
    *,
    ...
    if strategy == "medoid":
        selected = _select_medoid_indices(prototype_matrix, prototype_rows)
    elif strategy == "all":
        selected = tuple(range(prototype_matrix.shape[0]))
    else:
        raise VPMValidationError("prototype strategy must be 'medoid' or 'all'")

    vectors = np.ascontiguousarray(prototype_matrix[list(selected)], dtype=np.float32)
    ...
    manifest = VisualAddressManifest(
        ...
        metadata={"calibration_quantile": float(calibration_quantile)},
    )
...
class VectorAddressIndex:
    ...
    def match_vector(
        ...
        *,
        ...
        query_matrix = _as_matrix(np.asarray(vector, dtype=np.float32).reshape(1, -1), name="query vector")
        if query_matrix.shape[1] != self._matrix.shape[1]:
            raise VPMValidationError("query vector dimension does not match address index")
        norm = float(np.linalg.norm(query_matrix[0]))
        contract = self.contract()
        representation_digest = _representation_digest(
            query_matrix[0], contract.representation_spec_digest
        )
        if norm <= 0.0:
        ...
        second = int(conflict_candidates[0]) if conflict_candidates else int(ranking[1])
        nearest_score = float(similarities[first])
        second_score = float(similarities[second])
        margin = nearest_score - second_score
        threshold = float(self.build.calibration.acceptance_thresholds[nearest_row])
        required_margin = float(self.build.calibration.ambiguity_margins[nearest_row])
        similarity_ok = nearest_score + 1e-12 >= threshold
        margin_ok = margin + 1e-12 >= required_margin
...
class FrozenVectorAddressProvider:
    ...
    def __init__(self, encoder: FrozenVisualEncoder, index: VectorAddressIndex) -> None:
        ...
        if manifest.output_dimension != index._matrix.shape[1]:
            raise VPMValidationError("encoder output dimension does not match address index")
    ...
    def encode_batch(self, observations: Sequence[ImageObservation]) -> np.ndarray:
        ...
        for observation in items:
            ...
            if array.shape[:2] != (self.height, self.width):
                raise VPMValidationError("pixel observation shape violates encoder contract")
            if array.ndim == 3:
...
class LinearProbeIndex:
    ...
    def contract(self) -> VisualAddressContract:
        return VisualAddressContract(
            ...
            replay_contract="exact_decision",
            metadata={"encoder_manifest_id": self.build.encoder_manifest_id},
        )

    def match_vector(self, vector: Any, *, observation_digest: str) -> VisualAddressDecision:
        ...
        ranking = sorted(
            ...
            key=lambda index: (-float(scores[index]), self.build.row_ids[index]),
        )
        first = ranking[0]
        action = self.build.action_ids[first]
        conflicts = [index for index in ranking[1:] if self.build.action_ids[index] != action]
        second = conflicts[0] if conflicts else ranking[1]
        nearest = float(scores[first])
        second_score = float(scores[second])
        margin = nearest - second_score
        row_id = self.build.row_ids[first]
        threshold = float(self.build.calibration.acceptance_thresholds[row_id])
        required_margin = float(self.build.calibration.ambiguity_margins[row_id])
        accepted = nearest + 1e-12 >= threshold and margin + 1e-12 >= required_margin
        contract = self.contract()
        return VisualAddressDecision(
            accepted=accepted,
            ...
            observation_digest=str(observation_digest),
            representation_digest=_representation_digest(query, contract.representation_spec_digest),
            provider_kind=contract.provider_kind,
            provider_version=contract.provider_version,
            score_semantics="similarity",
            address_artifact_id=contract.address_artifact_id,
            ...
            second_row_id=self.build.row_ids[second],
            second_score=second_score,
            ambiguity_measure=margin,
            matched_row_id=row_id if accepted else None,
            accepted_by=("class_score_threshold", "conflicting_action_margin") if accepted else (),
            trace={"acceptance_threshold": threshold, "required_conflicting_action_margin": required_margin},
        )
...
def build_linear_probe(
    *,
    ...
    if set(rows) != set(str(value) for value in calibration_row_ids):
        raise VPMValidationError("linear probe prototype and calibration rows must match")
    action_by_row: Dict[str, str] = {}
    for row_id, action_id in zip(prototype_row_ids, prototype_action_ids):
        key = str(row_id)
        value = str(action_id)
    ...
    regularizer[-1, -1] = 0.0
    weights = np.linalg.solve(x_aug.T @ x_aug + regularizer, x_aug.T @ y).astype(np.float32)

    calibration_scores = np.concatenate(
        (calibration_x, np.ones((calibration_x.shape[0], 1), dtype=np.float32)), axis=1
    ) @ weights
    score_rows = np.eye(len(rows), dtype=np.float32)
    # Calibrate through the same generic logic by treating one-hot class-score
    # vectors as prototypes. This preserves per-row score and conflicting-action margins.
    calibration = _calibrate_prototypes(
        score_rows,
        rows,
    ...
    blob = MatrixBlob.from_array(
        weights,
        dtype="float32",
        metadata={"kind": "visual_linear_probe", "ridge": float(ridge)},
    )
    model_id = hashlib.sha256(
        b"zeromodel.linear-probe.identity.v1\0"
